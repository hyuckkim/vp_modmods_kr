...
--=====================================================================
-- FW Splash Missile (levels 1~5) — radius 1 stable, center=0, bDelay-safe
print("[FW] Splash missile (1~5) v6 loaded")

-- Do NOT redeclare SPECIALUNIT_MISSILE here (defined earlier in the file)
local iUnitVaultRack = GameInfoTypes["UNIT_FW_VAULT_RACK"]

-- Promotion type names (must match DB)
local PROMO_TYPES = {
  [1] = "PROMOTION_FW_SPLASH_DAMAGE_1",
  [2] = "PROMOTION_FW_SPLASH_DAMAGE_2",
  [3] = "PROMOTION_FW_SPLASH_DAMAGE_3",
}
local PROMO_IDS = {}; for i=1,5 do PROMO_IDS[i] = GameInfoTypes[PROMO_TYPES[i]] end

-- Unit-type → highest splash level map (fallback when pUnit is nil)
local UNIT_SPLASH_LEVEL = (function()
  local best = {}
  for lvl = 1, 5 do
    local pType = PROMO_TYPES[lvl]
    if pType and GameInfoTypes[pType] then
...
-- 타입 강제 레벨(예: GOD_ROD/2는 L5로 취급)
local UNIT_FORCE_LEVEL = {
  [GameInfoTypes.UNIT_FW_GOD_ROD]  = 5,
  [GameInfoTypes.UNIT_FW_GOD_ROD2] = 5,
}


-- Tuning: radius 1 only (center=0, ring1 damage per level)
local LEVELS = {
  [1] = { radius = 2, center = 0, ring = { 20, 10 } },
  [2] = { radius = 2, center = 0, ring = { 25, 12 } },
  [3] = { radius = 2, center = 0, ring = { 30, 15 } },
}
...
-- Ring-1 neighbors (no PlotIterators dependency)
local dirs = {
  ...
  for r=1, R do
    ...
    for _,p in ipairs(frontier) do
      for _,d in ipairs(dirs) do
        ...
        if q and not visited[q] then visited[q]=true; table.insert(nxt,q); table.insert(ring,q) end
      end
    end
...
-- Apply damage to units/city on a plot
...
-- Highest level from instance (if available)
...
local function OnUnitPrekill(iPlayer, iUnitID, iUnitType, iX, iY, bDelay, iByPlayer)
  ...
  _fired[key] = true
  print(string.format("[FW] Splash L%d @(%d,%d) r=1 ring1=%d", lvl, iX, iY, (cfg.ring and cfg.ring[1]) or 0))
end
