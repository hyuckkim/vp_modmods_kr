...
-- Cyberpolice 
-----------------------------------------------------------------
print("[Future building DUmmy] loadibg")
...
do
  ...
  if not iDummyBiomod then print("[Biomod][ERROR] Dummy BUILDING_DUMMY_BIOMOD_POP not found"); return end
  if not iTriggerBiomod then print("[Biomod][ERROR] Trigger BUILDING_FW_BIOMOD_TANK not found"); return end
  print("[Biomod] OK dummy=", iDummyBiomod, " trigger=", iTriggerBiomod, " per=", POP_PER_STACK, " cap=", STACK_CAP)
...
do
  ...
  if not iDummy then print("[Ecto][ERROR] Dummy BUILDING_DUMMY_MIL10_STACK not found"); return end
  if not iTrigger then print("[Ecto][ERROR] Trigger BUILDING_FW_ECTOGENESIS_POD not found"); return end
...
-- ──[약탈 차단/복구]────────────────────────────────────────
-- (A) VP/CP가 제공할 때: 약탈 커맨드 자체 차단
if GameEvents.CanDoCommand then
  GameEvents.CanDoCommand.Add(function(iPlayer, eCommand, iData1, iData2, iData3, bTestVisible)
    if eCommand ~= CommandTypes.COMMAND_PILLAGE then return end
    local p = Players[iPlayer]; if not p then return end
    local u = p:GetUnitByID(iData1); if not u then return end
...
-- (B) 모든 버전: 라퓨타 보유자의 턴 시작에 자국 영토 약탈 즉시 복구
local function AutoRepair(iPlayer)
