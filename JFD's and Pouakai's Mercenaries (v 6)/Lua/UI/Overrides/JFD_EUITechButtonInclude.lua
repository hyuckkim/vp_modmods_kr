...
function AddSmallButtonsToTechButton( thisTechButtonInstance, tech, maxSmallButtons, textureSize, startingButtonNum )
	...
	for row in GameInfo.Units( thisPrereqTech ) do
		if validUnitBuilds[row.Class] == row.Type and row.ShowInPedia and not addSmallArtButton( AdjustArtOnGrantedUnitButton, row ) then
			break
		end
	end
	...
	for row in GameInfo.Improvement_TechYieldChanges( thisTechType ) do
		local civilizationType = GameInfo.Improvements[row.ImprovementType].CivilizationType -- JFD Hide Irrelevant Unique Improvements
		if civilizationType and civilizationType == civType then
			improvementTypesChanged[ row.ImprovementType or -1 ] = true
		elseif (not civilizationType) then
			improvementTypesChanged[ row.ImprovementType or -1 ] = true
		end
	end
	...
	for row in GameInfo.Improvement_TechNoFreshWaterYieldChanges( thisTechType ) do
		...
		local isHidden = false -- JFD Hide Irrelevant Unique Improvements
		local civilizationType = GameInfo.Improvements[row.ImprovementType].CivilizationType
		if civilizationType and civilizationType ~= civType then isHidden = true end
		if yield and not isHidden and not addSmallActionButton( GameInfo.Builds{ ImprovementType = row.ImprovementType }(), yield.IconString,
	...
	if civ5_mode then
		for row in GameInfo.Technologies_JFD_MiscEffects(thisTechType) do
			if row.UnitAction then	
				addSmallActionButton( GameInfo.Missions[row.UnitAction], "", row.EffectToolTip )
			else
				addSmallGenericButtonIf(row.EffectToolTip and row.EffectToolTip )
			end
		end
