...
do
	...
	R.WHATEVER = BC("Sorry, we don't have time to discuss this right now...");
...
do 
	...
	local b_make_peace = B{
		...
		speech = "Let's negotiate a peace deal!" ;
...
--			{"TXT_KEY_DIPLO_NEGOTIATE_PEACE_BLOCKED_TT", rC.var1 }, FT{ STORE{ HumanTeam:GetNumTurnsLockedIntoWar(EnemyTeam), rC.var1 } ; rC.Nequal{ rC.var1 , 0 } ; } ;
	...
	local b_trade = B{
		...
		speech = "We would like to make a trade deal..." ;
	...
	local b_demand_help = B{
		...
		speech = "We need something from you..." ;
	...
	local b_share_plot = B{
		...
		speech = "There is a plot against you, that we would like you to know about..." ;
	...
	local b_stop_religion = B{
		text = "*Stop converting our cities!*" ;
	...
	local b_stop_digging = B{
		text = "*Stop digging in our lands!*" ;
	...
	local b_denounce = B{
		...
		headline = "[COLOR_WARNING_TEXT]Denouncement:[ENDCOLOR]" ;
	...
	local coopWarFilter = function( target )
		...
		if show then
			...
			if poss then
				...
				not_poss_reason = "[COLOR_WARNING_TEXT]Can't target this player: " .. (not_poss_reason or "?") .. "[ENDCOLOR]" ;
			end
	...
	local b_askWar = B{
		text = "Ask for a coop war..." ;
		action = function() displayLeaderPicker( coopWarFilter, coopCallback, "Pick coop war target.." ) end ;
	...
	local shareOpinionFilter = function( target )
		...
		if show then
			...
			if poss then 
				not_poss_reason = "What do you think of " .. Players[target]:GetNameOrNick() .. "?" ;
			else
				not_poss_reason = "[COLOR_WARNING_TEXT]Can't ask about this player: " .. (not_poss_reason or "?") .. "[ENDCOLOR]" 
			end
			return true, poss, not_poss_reason ;
		end
		return false ;
