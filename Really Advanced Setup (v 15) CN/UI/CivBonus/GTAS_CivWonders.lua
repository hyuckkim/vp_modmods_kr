...
function BuildControls()
	...
	table.insert(wonders, 1, { id = RANDOM_WONDER, description = Locale.ConvertTextKey("TXT_KEY_GTAS_RANDOM_WONDER") });
	...
	elseif WonderID == RANDOM_WONDER then
		Controls.WonderPulldown:GetButton():LocalizeAndSetText("TXT_KEY_GTAS_RANDOM_WONDER");
	else
		Controls.WonderPulldown:GetButton():LocalizeAndSetText("TXT_KEY_GTAS_WONDER_ERROR");
	end
	...
	if MapData.maxWonderCopies < 0 then
		Controls.MaxCopiesValue:LocalizeAndSetText("TXT_KEY_GTAS_WONDER_UNLIMITED");
	elseif MapData.maxWonderCopies == 1 then
		Controls.MaxCopiesValue:LocalizeAndSetText("TXT_KEY_GTAS_WONDER_SINGLE");
	else
		Controls.MaxCopiesValue:LocalizeAndSetText(tostring(MapData.maxWonderCopies));
	end
	...
	if slot:HasWonders() then
		...
		for index, wonder in slot:WonderList() do
			...
			if info ~= nil then
				...
				instance.WonderName:LocalizeAndSetText("TXT_KEY_GTAS_RANDOM_WONDER");
			end
			...
			instance.WonderSprinkle:LocalizeAndSetText(wonder.addSprinkles and "TXT_KEY_YES_BUTTON" or "TXT_KEY_NO_BUTTON");
