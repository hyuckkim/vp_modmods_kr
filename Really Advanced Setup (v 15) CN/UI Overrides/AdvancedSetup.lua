...
function ValidateControls()
	...
	if isValid then
		if isWarning then
			Controls.StartLabel:SetColorByName("Gold_Medal");
		else
			Controls.StartLabel:SetColorByName("Beige_Black_Alpha");
		end
	else
		Controls.StartLabel:SetColorByName("Red_Black");
	end

	if MapData.isRandomWorldSize then
		Controls.CivsButton:LocalizeAndSetText("TXT_KEY_GTAS_A_START_PLAYERS");
	else
		Controls.CivsButton:LocalizeAndSetText("TXT_KEY_GTAS_B_START_PLAYERS", SlotData:GetSlotCount());
	end
end
...
function ValidateStart()
	...
	function AddHelpText(text)
		helpText = helpText .. newline .. Locale.ConvertTextKey(text);
		newline = "[NEWLINE]";
	end
	...
	if humanSlot ~= nil and not MapData.isRandomWorldSize then
		...
		if not isValid then
			AddHelpText("TXT_KEY_GTAS_VALIDATESTART_HELP_1");
		end
	end
	...
	if activeCivs == 0 then
		isValid = false;
		AddHelpText("TXT_KEY_GTAS_VALIDATESTART_HELP_2");
	end
	...
	if isValid then
		if activeCivs == 1 then
			isWarning = true;
			AddHelpText("TXT_KEY_GTAS_VALIDATESTART_HELP_3");
		elseif not MapData.isRandomWorldSize and activeCivs < SlotData:GetSlotCount() then
			isWarning = true;
			AddHelpText("TXT_KEY_GTAS_VALIDATESTART_HELP_4");
		end

		if MapData.disableAllNaturalWonders then
			isWarning = true;
			AddHelpText("TXT_KEY_GTAS_VALIDATESTART_HELP_5");
		end
		
		if GameData.disableNukes then
			isWarning = true;
			AddHelpText("TXT_KEY_GTAS_VALIDATESTART_HELP_6");
		end
	end

	-- If no warnings then let the player know that it's OK to start the game.
	if helpText == "" then
		helpText = "TXT_KEY_GTAS_VALIDATESTART_HELP_7";
	end

	return isValid, isWarning, Locale.ConvertTextKey(helpText);
end
