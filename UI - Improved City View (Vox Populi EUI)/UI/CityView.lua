print("Loading CityView.lua from 'UI - Improved City View (Vox Populi with EUI)' version 21.0")
------------------------------------------------------
-- City View
...
--todo BE
		...
		if civ5_mode then
			...
			for specialist in GameInfo.Specialists() do
				local gpuClass = specialist.GreatPeopleUnitClass
				...
				for pSpecialistInfo in GameInfo.Specialists() do
					...
					for slot = 1, numSS do
						if specialistSlotTable[slot].Type == pSIType then
							local slotbuilding = specialistSlotTable[slot].Building;
							local slotfilled = specialistSlotTable[slot].Filled;
							local slotnum = specialistSlotTable[slot].Slot;
							STCount = STCount + 1;
							SCount = SCount + 1;
							local controlTable = g_SpecialistSlotIM:GetInstance();
					...
					if STCount > 0 then
						local controlTable = g_SpecialistIconIM:GetInstance();
		...
		for gwt, gwall in pairs(greatWorksData) do -- 3 types
			...
			for i, gwdata in pairs(gwall) do -- specific type
				...
				-- create an instance
				local gwInstance = g_GreatWorksSlotIM:GetInstance();
...
-- Maggi: Specialists
StackInstanceManager("", "", Controls.SpecialistsStack, Controls.SpecialistsHeader, ResizeRightStack)
-- Infixo Great Works
StackInstanceManager("", "", Controls.GreatWorksStack, Controls.GreatWorksHeader, ResizeRightStack)
