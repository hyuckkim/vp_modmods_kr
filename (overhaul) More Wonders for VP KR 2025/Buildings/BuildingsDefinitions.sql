...
VALUES		('TXT_KEY_PROMOTION_FLETCHER',		'독화살'),
			('TXT_KEY_PROMOTION_FLETCHER_HELP',	'부상당한 유닛 공격 시 [ICON_RANGE_STRENGTH] 공격 전투력 +50%.');
...
VALUES		('TXT_KEY_BUILDING_FLETCHER',			'화살 제조소'),
			('TXT_KEY_BUILDING_FLETCHER_PEDIA',		'TODO'),
			('TXT_KEY_BUILDING_FLETCHER_HELP',		'생산된 궁술 유닛에 [COLOR_POSITIVE_TEXT]독화살[ENDCOLOR] 승급을 부여합니다. 이 도시의 궁술 유닛 생산력 +10%.[NEWLINE][NEWLINE]인접한 [ICON_RES_TROPICAL_FISH] 열대어: [ICON_PRODUCTION] 생산력 +2.'),
			('TXT_KEY_BUILDING_SEAFOOD',			'해산물 시장'),
			('TXT_KEY_BUILDING_SEAFOOD_PEDIA',		'TODO'),
			('TXT_KEY_BUILDING_SEAFOOD_HELP',		'도시의 [ICON_FOOD] 식량 5%가 매 턴 [ICON_TOURISM] 관광으로 전환됩니다. [ICON_GREAT_WORK] 위대한 음악 작품 슬롯 1개를 포함합니다.[NEWLINE][NEWLINE][ICON_CULTURE] 지루함으로 인한 [ICON_HAPPINESS_3] 행복 -1.[NEWLINE][NEWLINE]인접한 [ICON_RES_FISH] 물고기: [ICON_FOOD] 식량 +1.[NEWLINE]인접한 [ICON_RES_CRAB] 게: [ICON_FOOD] 식량 +1, [ICON_TOURISM] 관광 +1.[NEWLINE]인접한 [ICON_RES_TROPICAL_FISH] 열대어: [ICON_FOOD] 식량 +1, [ICON_TOURISM] 관광 +1.');
...
UPDATE Language_ko_KR SET Text = Text||'[NEWLINE]인접한 [ICON_RES_SHRIMP] 새우: [ICON_FOOD] 식량 +1, [ICON_TOURISM] 관광 +1.' WHERE Tag = 'TXT_KEY_BUILDING_SEAFOOD_HELP' AND EXISTS (SELECT * FROM COMMUNITY WHERE Type='MW-CIV-LOI' AND Value= 1);
