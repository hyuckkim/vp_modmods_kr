...
-- 산업시대 중기병 "흉갑기병", VP에서 하르케부스병으로 변경됨
...
UPDATE Language_ko_KR  SET
Text = '하르케부스병은 카빈총의 일종을 사용하는 것이 특징이었는데, 가장 초기에 사용된 카빈총은 ''하르케부스''라고 불렸습니다(이 단어는 더 무거운 보병용 무기인 아르케부스에서 유래했습니다).이 기병들은 가슴과 등판이 달린 철제 흉갑과 가재 꼬리 모양의 솥처럼 얼굴이 드러나는 투구만 착용한 가벼운 갑옷을 입고 있었다. 이 때문에 철제 흉갑이 고급 가죽 코트보다 일반적으로 저렴했기 때문에 군복을 갖추는 데 비용이 많이 들지 않았습니다. 16세기 후반부터 다음 세기 초 수십 년까지, 하르케부스병은 그보다 앞선 시대의 페트로넬과 마찬가지로 반창기병이나 권총 기병과 같은 중무장 기병을 지원하는 역할을 맡았습니다.'
...
-- 곡사포 (VP용 야전포)
REPLACE INTO Language_ko_KR (Tag, Text) VALUES
...
('TXT_KEY_PROMOTION_FASIMBA_HELP', '[COLOR_POSITIVE_TEXT]화약 유닛[ENDCOLOR]에 대해 [ICON_STRENGTH] 공격력 +33% 증가.');
...
('TXT_KEY_WONDER_EE_FASIL_GHEBBI_HELP', '파실 게비 주변 3타일 이내에 있는 아군 군사 유닛의 [ICON_STRENGTH] 전투력이 +25% 증가합니다.'),
...
('TXT_KEY_WONDER_EE_KRONBORG_HELP', '모든 해안 도시에서 체력이 +50 증가하고, [ICON_STRENGTH] 방어력이 +3 증가하며, 군사 유닛 보급 한도가 +1 증가합니다.'),
...
('TXT_KEY_WONDER_EE_ROYALSOC_HELP', '모든 도시의 [ICON_RESEARCH] 문맹으로 인한 [ICON_HAPPINESS_3] 불행이 [COLOR_POSITIVE_TEXT]10%[ENDCOLOR] 감소합니다.'),
...
('TXT_KEY_WONDER_EE_TOPKAPI_HELP', '가능한 테마 보너스:[NEWLINE][ICON_BULLET][COLOR_POSITIVE_TEXT]다른 문명[ENDCOLOR]과 [COLOR_CYAN]연속적인 시대[ENDCOLOR]의 [ICON_GW_ART] 걸작 예술품과 [ICON_ARTIFACT] 유물에서 [ICON_CULTURE] 문화 +8, [ICON_TOURISM] 관광 +8을 얻습니다.'),
...
('TXT_KEY_BUILDING_EE_TAVERN_PEDIA', '알코올 음료를 판매하고 소비하는 시설은 인류 문명 전반에 걸쳐 도시의 중요한 요소였습니다. 대항해 시대가 도래하면서 이러한 시설들은 항구 도시와 해안 정착지에서 점점 더 중요한 역할을 하게 되었습니다. 이곳들은 미지의 땅으로 향하는 길고 위험한 항해를 앞둔 선원, 상인, 탐험가들에게 필수적인 집결지가 되었습니다. 이러한 태번은 선원이 필요한 선박들을 위한 비공식적인 모집 센터 역할을 했으며, 해양 정보와 항해 전문 지식을 교환하는 중심지 역할을 했습니다. 이러한 시설들은 단순히 나그네와 도둑뿐만 아니라 정치적 불만 세력과 모험가들에게도 안식처 역할을 했습니다.[NEWLINE][NEWLINE]16세기 중반에 이르러서는 모든 계층의 도시 사람들 사이에서 외식 문화가 확고하게 자리 잡았다. 태번은 조수와 바람에 따라 생활하는 떠돌이 손님들의 필요를 충족시키기 위해 정해진 시간에 매일 식사를 제공하는 관습을 시작했습니다. 태번이 사회적으로 점차 용인되면서, 괜찮은 태번은 정기적인 모임 장소이자 비공식적인 클럽하우스 역할을 하여 선원과 탐험가들 사이의 동료애를 키우고 대항해 시대에 매우 중요했던 정보 교환을 촉진했습니다.'),
-- 방직소
