...
function MapGlobals:New()
	...
	mglobal.iceLatitude			= 77	-- Furthest down ice is allowed to go
	...
	mglobal.landMaxScatter			= 0.085	--Recommended range:[0.03 to 0.3]	--and very high values result in a lot of stringy continents and islands.
											
											
	mglobal.coastScatter			= 0.054 	--Recommended range:[0.01 to 0.3]
						--Higher values result in more islands and variance on landmasses and coastlines.
											
	mglobal.mountainScatter			= 300 * mapW --Recommended range:[130 to 1000]
	...
	mglobal.belowMountainPercent		= 0.951	-- Percent of non-mountain land
										   -- flatPercent to belowMountainPercent : hills
	mglobal.flatPercent			= 0.8 	-- Percent of flat land
	...
	mglobal.junglePercent		= 0.60 	-- Base percent chance for jungle to spawn on suitable tile (will be more on 'perfect' tile) 
	mglobal.forestPercent		= 0.086		-- Base percent chance for forest to spawn on suitable tile (will be more on 'perfect' tile) 
	mglobal.forestTundraPercent	= 0.315		-- Base percent chance for marsh to spawn on tundra tile (half on tundra hills) 
	...
	mglobal.jungleClumpingModifier	= 2		-- chance increase based on percent of nearby tiles which have jungle
	mglobal.forestClumpingModifier	= 26		-- chance increase based on percent of nearby tiles which have jungle
	...
	mglobal.grassPercent	        = 0.745	--	> grassPercent: very wet (more jungle/marsh, less forest)
						-- 	> plainsPercent: grass
	mglobal.plainsPercent		= 0.435 	-- 	desertPercent to plainsPercent: plains/tundra/snow
	mglobal.desertPercent		= 0.09 	--	< desertPercent : desert if hot enough, plains/tundra/snow if not
	
	
	-- Temperature
	mglobal.jungleMinTemperature	= 0.76 -- > jungleMin: increased jungle, less marsh or forests.
	mglobal.desertMinTemperature	= 0.56 -- > desertMin: desert and jungle 
	mglobal.tundraTemperature	= 0.35 -- < tundraTemp: tundra
	mglobal.snowTemperature		= 0.175 -- < snowTemp:   snow
	...
	mglobal.minWaterTemp			= 0.11	-- Sets water temperature compression that creates the land/sea seasonal temperature differences that cause monsoon winds.
	...
	mglobal.oceanMaxWander			= 30	-- number of tiles a rift can randomly wander from its intended path
	...
	mglobal.atlanticBulge			= -0.09	-- size increase at equator
	mglobal.atlanticCurve			= 0.2	-- S-curve distance
	mglobal.pacificSize			= 0.04	-- size near poles
	mglobal.pacificBulge			= 0.07	-- size increase at equator
	...
	mglobal.elevationBlendRange		= 11 -- range to smooth elevation map
	...
	mglobal.northAttenuationRange	= 0.17 --percent of the map height.
	mglobal.southAttenuationFactor	= 0.75
	mglobal.southAttenuationRange	= 0.17
	...
	mglobal.pickUpModifier		= 0.9	-- percent of rain which falls on land and is picked up again (depends on temperature)
	mglobal.rainfallMultiplier	= 1.5	-- amount of moisture dropped per percentage change in pressure
	mglobal.basePressure		= 2	-- pressure at 0 elevation (pressure decreases as elevation increases) (keep this above 1)
...
function GetMapScriptInfo()
	...
	return {
		Name = "Milae's Map v14",
		Description = "Milae's modifications to the Communitas Map, new rainfall + feature system",
		IsAdvancedMap = 0,
...
function GetMapInitData(worldSize)
	...
	local worldsizes = {
		[GameInfo.Worlds.WORLDSIZE_DUEL.ID] = {43, 28},
		[GameInfo.Worlds.WORLDSIZE_TINY.ID] = {55, 36},
		[GameInfo.Worlds.WORLDSIZE_SMALL.ID] = {66, 43},
		[GameInfo.Worlds.WORLDSIZE_STANDARD.ID] = {75, 49},
		[GameInfo.Worlds.WORLDSIZE_LARGE.ID] = {83, 54},
		[GameInfo.Worlds.WORLDSIZE_HUGE.ID] = {92, 60}
		}
