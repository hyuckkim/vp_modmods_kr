print("This is the 'UI - Map Pins' mod script.")
  ...
  if (iPin ~= nil) then
    ...
    if (pin ~= nil and pin ~= 0) then
      if (pin.id ~= nil) then
        ...
        DbmExec(string.format(modDbDelete, pin.id))
      end
    else
...
local bPinContainerHidden
function OnEnterCityScreen()
  bPinContainerHidden = Controls.PinContainer:IsHidden()
  Hide()
end
Events.SerialEventEnterCityScreen.Add(OnEnterCityScreen)

function OnExitCityScreen()
  Controls.PinContainer:SetHide(bPinContainerHidden)
end
