...
function CheckVicinity(minDistance, type, value, x, y)
	...
	if type == "StartingPlot" then
		if checkNum > 0 then
			...
			return checkNum, checkNum >= minDistance
		else
			--print ("vicinity : none")
			return false
		end
...
function GetStartingPositionCiv(playerID, civID)
	local mapID = modUserData.GetValue ("MapID")
	tableName = "Civilizations_" .. g_MapList[mapID].TablePrefix .. "StartPosition"
	print("check starting positions")
	for row in GameInfo[tableName]() do
		if MajorCivExist ( row.Type ) then
			if civID == GameInfo["Civilizations"][row.Type]["ID"] then
				...
				local isValidPlot = IsValidPlot(playerID, 3, plot)
				if (not isValidPlot) then
					plot = Map:GetPlotXY(row.AltY, row.AltX)
					isValidPlot = IsValidPlot(playerID, 3, plot)
				end
				if (not isValidPlot) then
					return false
				end	
	...
	for row in GameInfo.Civilizations_YnAEMP("MapPrefix = '" .. g_MapList[mapID].TablePrefix .. "'") do
		if MajorCivExist ( row.CivilizationType ) then
			if civID == GameInfo["Civilizations"][row.CivilizationType]["ID"] then
				...
				print (" Civtype : " .. row.CivilizationType)
				-- X,Y still inversed...
				local plot = Map:GetPlotXY(y, x)
				CheckVicinity(10, "StartingPlot", 0, y, x)
				local isValidPlot = IsValidPlot(playerID, 3, plot)
				...
				print("valid plot is:", plot:GetX(), plot:GetY())
				if (not isValidPlot) then
					return false
				end	
				return plot
...
function GetStartingPositionMinorCiv(minorCivID)
	...
	for row in GameInfo.MinorCivilizations_YnAEMP("MapPrefix = '" .. g_MapList[mapID].TablePrefix .. "'") do
		if MinorCivExist(row.MinorCivType) then
			if minorCivID == GameInfo["MinorCivilizations"][row.MinorCivType]["ID"] then
				-- Bosphorus case
				local x = row.X
				local y = row.Y 
				if (g_BosphorusPlot[mapID]) then
				...
				print (" MinorCivtype : " .. row.MinorCivType)
				-- X,Y still inversed...
				local plot = Map:GetPlotXY(y, x)
				local numCheck
				local wantedSeparation = g_MinorDistanceValue[modUserData.GetValue("MinorDistance")]
...
function PopulateWorld ()
	...
	if modUserData.GetValue("MajorPlacement") ~= MAJOR_ALL_RANDOM then
		for i = 0, GameDefines.MAX_MAJOR_CIVS - 1 do
			...
			local plot = GetStartingPositionCiv(i, civID)
			if plot then
				print ("Found starting position for Civ : " .. civID .. " X:" .. plot:GetX() .. " Y:" .. plot:GetY())
	...
	for i = 0, GameDefines.MAX_CIV_PLAYERS-1, 1 do
		...
		if player:IsAlive() then
			...
			if startPlot then
				...
				for row in GameInfo.Civilizations_YnAEMPRequestedResources("MapPrefix ='" .. g_MapList[mapID].TablePrefix .. "'") do
					local i = 1
					local reqResource = row["Req" .. i]
					while reqResource do
						if ResourceExist ( reqResource ) then
