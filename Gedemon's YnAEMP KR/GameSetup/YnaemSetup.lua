...
g_MapOptionsManager = InstanceManager:new("MapOptionInstance", "MapOptionRoot", Controls.MapOptionsStack);
...
function RefreshCheckBoxGameOptions()
	g_MapOptionsManager:ResetInstances();
	...
	for option in GameInfo.GameOptions{Visible = 1} do
		...
		if option.Type ~= "GAMEOPTION_DISABLE_START_BIAS" then
			...
			if ( option.Type == "GAMEOPTION_COMPLETE_KILLS" ) and ( g_dynamicCS == 1 ) then
				...
				optionToolTip = "동적인 역사에는 완전한 처치 설정이 필수입니다."
				optionChecked = 1
				optionDisabled = true
			...
			elseif ( option.Type == "GAMEOPTION_NO_BARBARIANS" ) and ( g_dynamicCS == 1 ) then
				...
				optionToolTip = "동적인 역사는 사용자 정의된 야만인 생성을 사용합니다."
				optionChecked = 1
				optionDisabled = true
				PreGame.SetGameOption(option.Type, 1)
				modUserData.SetValue ("GameOption_"..option.Type, 1)
			-- elseif option.IsMapOption then
				-- optionToolTip = (option.Help) and Locale.ConvertTextKey(option.Help) or nil
				-- optionChecked = ((option.Default == 1) or option.Default) and true or false
				-- optionDisabled = false	
				-- PreGame.SetGameOption(option.Type, optionChecked)
				-- print(option.Type, PreGame.GetGameOption(option.Type))
			else
				optionToolTip = (option.Help) and Locale.ConvertTextKey(option.Help) or nil
				optionChecked = (option.Default == 1) and true or false
				optionDisabled = false
			end
			if (option.IsMapOption and (not IsCulDivActive())) then
				optionChecked = false
				optionDisabled = true
				optionToolTip = "[COLOR_POSITIVE_TEXT]JFD의 문화적 다양성[ENDCOLOR] 모드를 사용하지 않기 때문에 이 설정을 변경할 수 없습니다."
			end
			local option = {
				Type = option.Type,
				...
				GameOption = true,
				SortPriority = 10,
				IsMapOption = option.IsMapOption,
			}
	...
	for _, option in ipairs(options) do
		local gameOption = nil 
		if option.IsMapOption then 
			gameOption = g_MapOptionsManager:GetInstance();
			gameOption = gameOption.MapOptionRoot
		else
			gameOption = g_GameOptionsManager:GetInstance();
			gameOption = gameOption.GameOptionRoot
		end
		
		local gameOptionTextButton = gameOption:GetTextButton();
		gameOptionTextButton:SetText(option.Name);
								
		if(option.ToolTip ~= nil) then
			gameOption:SetToolTipString(option.ToolTip);
		end
		
		gameOption:SetDisabled(option.Disabled);
		gameOption:SetCheck(option.Checked);
		sizeY = sizeY + gameOption:GetSizeY();
		if(option.GameOption == true) then
			gameOption:RegisterCheckHandler( function(bCheck)
				PreGame.SetGameOption(option.Type, bCheck);
				local value = 0
				if bCheck then value = 1 end
				modUserData.SetValue ("GameOption_"..option.Type, value)
				if option.IsMapOption then PerformCivFullSync() end
			end);
		else
			gameOption:RegisterCheckHandler( function(bCheck)
	...
	Controls.AdvancedMapScrollPanel:CalculateInternalSize();
	Controls.VictoriesScrollPanel:CalculateInternalSize();
	Controls.OptionsScrollPanel:CalculateInternalSize();
end
------------------------------------------------------------------------------------------------------
function OnAllCulturesChecked(isChecked)
	if isChecked then
		PreGame.SetGameOption("GAMEOPTION_CULTURE_JFD_ALL", 1)
	else
		PreGame.SetGameOption("GAMEOPTION_CULTURE_JFD_ALL", 0)
	end
	ScreenOptions["Civs"].FullSync();
end
Controls.AllCultures:RegisterCheckHandler(OnAllCulturesChecked)
------------------------------------------------------------------------------------------------------
		...
		function GetPlayableCivInfo()
			...
			for row in DB.Query(sql) do
				...
				if CivHasTSL(row.CivID) or (PreGame.GetMapOption(MAJOR_PLACEMENT) ~= MAJOR_TSL_ONLY) then
					if CivCultureIsValid(row.CivID) then
						table.insert(civs, {
							CivID = row.CivID,
			...
			table.sort(civs, function(a, b) return a.ShortDescription < b.ShortDescription end);	
...
["Hotseat"] = {
	FullSync = function()
		function PopulateHotseatPulldown( pullDown, playerID )
			...
			pullDown:BuildEntry( "InstanceOne", controlTable );
			controlTable.Button:LocalizeAndSetText("인간");
			controlTable.Button:SetVoids( playerID, 1 );
			...
			pullDown:RegisterSelectionCallback(function(playerID, playerChoiceID)
				...
				if g_Hotseat[playerID] then
					if( slotInstance ~= nil ) then
						slotInstance.HotseatLabel:LocalizeAndSetText( "인간" );
					else
						Controls.HotseatLabel:LocalizeAndSetText( "인간" );
					end
				else
					if( slotInstance ~= nil ) then
	...
	PartialSync = function()
		for i = 1, GameDefines.MAX_MAJOR_CIVS-1, 1 do -- i = 0 is always human
			if g_Hotseat[i] then
				g_SlotInstances[i].HotseatLabel:LocalizeAndSetText( "인간" );
			else
...
function OnStartClicked()
		...
		if i < modUserData.GetValue ("NumPlayers") then
			...
			if g_Hotseat[i] then
				...
				if( civIndex ~= -1 ) then
					...
					PreGame.SetNickName( i, "플레이어 " .. i+1 )
				end
			else			
	...
	if ( g_bBadCS ) then
		Controls.StartButton:SetText("[COLOR_WARNING_TEXT]모드 오류 ![ENDCOLOR]")
		Controls.StartButton:SetToolTipString("다음 모드 중 하나만 활성화해야 합니다:[NEWLINE]-Ynaemp 도시 국가[NEWLINE]-Ynaemp 도시 국가 확장[NEWLINE]-Ynaemp 도시 국가 마데이라[NEWLINE]-Ynaemp 동적인 역사[NEWLINE]모드 선택 메뉴로 돌아가서 불필요한 모드의 선택을 모두 해제하세요.")
	else
