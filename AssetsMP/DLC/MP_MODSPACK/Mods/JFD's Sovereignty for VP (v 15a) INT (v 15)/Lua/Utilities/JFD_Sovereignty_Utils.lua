...
function Player.CalculateSovereignty(player, updateCurrentSovereignty, returnsTT)	
    ...
    if numLegitimaciesMet >= 3 then 
        numSov = numSov + 15
        if returnsTT then
            strSovereigntyTT = strSovereigntyTT .. "[NEWLINE] [ICON_BULLET]3가지 [COLOR_POSITIVE_TEXT]정당성[ENDCOLOR]이 모두 충족되면 [COLOR_POSITIVE_TEXT]+15%[ENDCOLOR]." 
        end
    else
        if returnsTT then
            strSovereigntyTT = strSovereigntyTT .. "[NEWLINE] [ICON_BULLET]3가지 [COLOR_POSITIVE_TEXT]정당성[ENDCOLOR]이 모두 충족되면 +15%." 
        end
    end
