...
function GetResourceString(plot, bLongForm)
	...
	if (plot:GetResourceType(iActiveTeam) >= 0) then
		...
		if (resourceType == GameInfoTypes.RESOURCE_ARTIFACTS or resourceType == GameInfoTypes.RESOURCE_HIDDEN_ARTIFACTS) then
			local response = {Icon=pResource.IconString, Tip=convertedKey};
			LuaEvents.GetAntiquitiesTooltip(plot, bLongForm, iActiveTeam, resourceType, response);
			improvementStr = improvementStr .. response.Icon .. response.Tip;
		else
