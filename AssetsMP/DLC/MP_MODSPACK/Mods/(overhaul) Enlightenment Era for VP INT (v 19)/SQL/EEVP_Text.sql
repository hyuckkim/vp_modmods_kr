...
('TXT_KEY_UNIT_EE_FIELD_GUN_STRATEGY', '야전포는 계몽 시대의 포병 유닛입니다. 사거리는 업그레이드된 사석포와 동일하지만, 훨씬 더 강력합니다. 다른 공성 유닛과 마찬가지로 멀리 볼 수 없으므로 보호가 필요합니다. 적의 영토에서는 절반 속도로 이동하지만, 야전포는 계몽 시대 정복에 필수적입니다.'),
-- 갤리온
...
('TXT_KEY_UNIT_EE_GALLEON_STRATEGY', '갤리온은 계몽시대의 해군 원거리 유닛입니다. 갤리어스에서 업그레이드된 갤리온은 자유롭게 대양으로 진출할 수 있는 최초의 원거리 함선입니다. 더 큰 함선의 지원을 받는 갤리온 함대를 활용하여 해외 도시를 정복하고 해외 영토를 보호하세요.'),
-- 명사수
