print("This is the modded DiscussionDialog from 'UI - Diplomacy Log'")
...
function LeaderMessageHandler( iPlayer, iDiploUIState, szLeaderMessage, iAnimationAction, iData1 )
    ...
    if (bMyMode) then
		...
		Controls.RememberPanel:SetHide(false)
		Controls.RememberLeader:SetText(pAIPlayer:GetName())
		Controls.RememberOneCiv:SetCheck(false)
		Controls.RememberAllCivs:SetCheck(false)
		
		-- Human invoked discussion
		if (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_HUMAN_INVOKED) then
			Controls.RememberPanel:SetHide(true)
...
-- REMEMBER
----------------------------------------------------------------
function DoRemember(iFromUIDiploEvent, iButton)
	if (g_iAIPlayer ~= g_iStfuAI or g_iStfuDiploMessage == -1) then
		print("ERROR: Something bad happened in DiscussionDialog!")
	else
		if (iButton == -1) then
			if (Controls.RememberOneCiv:IsChecked()) then
				LuaEvents.Diplomacy_Ignore(g_iAIPlayer, g_iStfuDiploMessage)
			elseif (Controls.RememberAllCivs:IsChecked()) then
				LuaEvents.Diplomacy_Ignore(-1, g_iStfuDiploMessage)
			end
		else
			if (iFromUIDiploEvent ~= -1) then
				if (Controls.RememberOneCiv:IsChecked()) then
					LuaEvents.Diplomacy_Remember(g_iAIPlayer, g_iStfuDiploMessage, iFromUIDiploEvent, iButton)
				elseif (Controls.RememberAllCivs:IsChecked()) then
					LuaEvents.Diplomacy_Remember(-1, g_iStfuDiploMessage, iFromUIDiploEvent, iButton)
				end
			end
		end
	end
...
function OnButton1()
	...
	if (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_HUMAN_INVOKED) then
		if (g_iInvokedDiscussionMode == g_iModeDiscussionRoot) then
			...
			iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_HUMAN_DISCUSSION_SHARE_INTRIGUE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iIntriguePlotter, iIntrigueType);
		end
	...
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_AGGRESSIVE_MILITARY_WARNING) then
	    iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_AGGRESSIVE_MILITARY_WARNING_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, 0 );
    
    -- AI told us he attacked a Minor we're friends with - we forgive him
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_I_ATTACKED_YOUR_MINOR_CIV) then
		local iMinor = g_iDiploData;
	    iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_I_ATTACKED_YOUR_MINOR_CIV_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, iMinor );

    -- AI told us he bullied a Minor we have a PtP with - we forgive him
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_I_BULLIED_YOUR_MINOR_CIV) then
		local iMinor = g_iDiploData;
	    iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_I_BULLIED_YOUR_MINOR_CIV_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, iMinor );

    -- AI warned us about attacking a Minor - we tell him to go away
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_YOU_ATTACKED_MINOR_CIV) then
	    iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_ATTACKED_MINOR_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, 0 );
    
    -- AI scolded us for killing a Minor - we tell him to go away
	--elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_YOU_KILLED_MINOR_CIV) then
	    --iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_KILLED_MINOR_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, 0 );

    -- AI warned us about bullying a Minor - we tell him to go away
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_YOU_BULLIED_MINOR_CIV) then
	    iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_BULLIED_MINOR_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, 0 );

    -- AI seriously warning us about expansion - we tell him to go away
	--elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_YOU_EXPANSION_SERIOUS_WARNING) then
	    --iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_EXPANSION_SERIOUS_WARNING_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, 0 );
    
    -- AI warning us about expansion - we tell him to go away
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_YOU_EXPANSION_WARNING) then
	    iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_EXPANSION_WARNING_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, 0 );
    
    -- AI seriously warning us about plot buying - we tell him to go away
	--elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_YOU_PLOT_BUYING_SERIOUS_WARNING) then
	    --iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_PLOT_BUYING_SERIOUS_WARNING_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, 0 );
    
    -- AI warning us about plot buying - we tell him to go away
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_YOU_PLOT_BUYING_WARNING) then
	    iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_PLOT_BUYING_WARNING_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, 0 );
    
    -- AI asking to work together - we agree
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_WORK_WITH_US) then
	    iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_WORK_WITH_US_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, 0 );
	...
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_WORK_AGAINST_SOMEONE) then
	    ...
	    iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_WORK_AGAINST_SOMEONE_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, iAgainstPlayer );
        
    -- AI asking to declare war against someone - we say no
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_COOP_WAR) then
		local iAgainstPlayer = g_iDiploData;	-- This should be set when receiving the leader message
	    iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_COOP_WAR_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, iAgainstPlayer );
	    
    -- AI asking to declare war against someone NOW - we agree
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_COOP_WAR_TIME) then
		local iAgainstPlayer = g_iDiploData;	-- This should be set when receiving the leader message
	    iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_COOP_WAR_NOW_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, iAgainstPlayer );
    
    -- AI asking to make RA in the future - we agree
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_PLAN_RESEARCH_AGREEMENT) then
	    iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_PLAN_RA_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, 0 );
    
    -- AI asking to denounce another player - we agree
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_AI_REQUEST_DENOUNCE) then
		local iAgainstPlayer = g_iDiploData;	-- This should be set when receiving the leader message
	    iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_AI_REQUEST_DENOUNCE_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, iAgainstPlayer );
		
	-- AI asking to stop spying - we ignore
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_CAUGHT_YOUR_SPY) then
		iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_CAUGHT_YOUR_SPY_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, iAgainstPlayer);
	...
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_KILLED_MY_SPY) then
		iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_KILLED_MY_SPY_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, iAgainstPlayer);
	
	-- forgive the AI and mark the dialog as done
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_CONFRONT_YOU_KILLED_MY_SPY) then
		-- Faking the button type to be forgiven
		iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_KILLED_MY_SPY_RESPONSE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, 2, iAgainstPlayer);
		
    -- AI asking you to stop religious conversions	
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_STOP_CONVERSIONS) then
	   iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_STOP_CONVERSIONS; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, iAgainstPlayer);
	   
    -- AI asking you to stop digging
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_STOP_DIGGING) then
	   iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_STOP_DIGGING; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, iButtonID, iAgainstPlayer);
	...
	DoRemember(iFromUIDiploEvent, iButtonID)
end
...
function OnButton2()
    ...
    if (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_HUMAN_INVOKED) then
		if (g_iInvokedDiscussionMode == g_iModeDiscussionRoot) then
			...
			iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_HUMAN_DISCUSSION_STOP_SPYING; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, 0, 0 );
		end
	...
	elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_CONFRONT_YOU_KILLED_MY_SPY) then
		if (g_iInvokedDiscussionMode == g_iModeDiscussionRoot) then
			-- Ask the AI player not to spy any more
			iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_HUMAN_DISCUSSION_STOP_SPYING; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, 1, 0 );
		end
...
function OnButton3()
	...
	if (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_HUMAN_INVOKED) then
		if (g_iInvokedDiscussionMode == g_iModeDiscussionRoot) then
			-- Ask the AI player not to spy any more
			iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_HUMAN_DISCUSSION_STOP_SPREADING_RELIGION; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, 0, 0 );
		end
...
function OnButton4()
	...
	if (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_HUMAN_INVOKED) then
		if (g_iInvokedDiscussionMode == g_iModeDiscussionRoot) then
			...
			iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_HUMAN_DISCUSSION_DONT_SETTLE; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, 0, 0 );
			-- Tell the player we're done working with him
			--elseif (pAIPlayer:IsDoF(iActivePlayer)) then
				--iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_HUMAN_DISCUSSION_END_WORK_WITH_US; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, 0, 0 );
		end
...
function OnButton5()
	...
	if (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_HUMAN_INVOKED) then
		if (g_iInvokedDiscussionMode == g_iModeDiscussionRoot) then
			iFromUIDiploEvent = FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_HUMAN_DISCUSSION_STOP_DIGGING; Game.DoFromUIDiploEvent(iFromUIDiploEvent, g_iAIPlayer, 0, 0 );
		end
...
----------------------------------------------------------------        
----------------------------------------------------------------        
function OnRememberOneCiv(bIsChecked)
  if (bIsChecked) then
    Controls.RememberAllCivs:SetCheck(false)
  end
end
Controls.RememberOneCiv:RegisterCheckHandler(OnRememberOneCiv);

function OnRememberAllCivs(bIsChecked)
  if (bIsChecked) then
    Controls.RememberOneCiv:SetCheck(false)
  end
end
Controls.RememberAllCivs:RegisterCheckHandler(OnRememberAllCivs);
...
function OnDiplomacyStfu(iAI, iResponseType, iDiploMessage)
  ...
  g_iStfuAI = iAI
  g_iStfuDiploMessage = iDiploMessage
