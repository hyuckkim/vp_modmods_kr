...
function Game.CanRequestCoopWar( iMe, iPartner, iTarget )
	...
	if not pTarget:IsHasMet(iPartner) then
		return false, ("%s 지도자와 %s 지도자는 아직 만난 적이 없습니다!"):format( Players[iPartner]:GetNameOrNick(),pTarget:GetNameOrNick() ) ;
	end
	
	if pTarget:IsAtWarWith(iMe) then
		return false, ("이미 %s의 문명과 전쟁 중"):format( pTarget:GetNameOrNick() ) ;
	end
	
	if pTarget:IsAtWarWith(iPartner) then
		return false, ("이미 %s의 문명과 %s의 문명이 전쟁 중"):format(  Players[iPartner]:GetNameOrNick(),pTarget:GetNameOrNick() ) ;
	end

	if not Players[iMe]:CanRequestCoopWar(iPartner,iTarget) then
		return false, "DLL이 안 된다고 합니다!" ;
	end
	return true ;
end
...
function randomText( ... )
	...
	for i = 1 , select("#",...) do
		...
		if txt:find("RESPONSE_") == 1 then
			for row in DB.Query( "Select Tag,Response from Diplomacy_Responses, Language_ko_KR where LeaderType = 'GENERIC' and ResponseType = ? and Tag like Response", txt ) do
...
function randomTextS( ... )
	...
	for i = 1 , select("#",...) do
		local txt = select(i,...);
		if txt:find("RESPONSE_") == 1 then
			for row in DB.Query( "Select Tag from Diplomacy_Responses, Language_ko_KR where LeaderType = 'GENERIC' and ResponseType = ? and Tag like Response", txt ) do
				ret[#ret+1] = row.Tag ;
			end
		else
			ret[#ret+1] = txt ;
