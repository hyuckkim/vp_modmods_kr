...
INSERT OR REPLACE INTO Language_en_US 
		(Tag, 							 Text)
SELECT	'TXT_KEY_WONDER_BARNENEZ_HELP', '';
------------------------------------------------------------------------------------------------------------------------------------------
-- NEWGRANGE
------------------------------------------------------------------------------------------------------------------------------------------
UPDATE Buildings
...
INSERT OR REPLACE INTO Language_en_US 
		(Tag, 							 Text)
SELECT	'TXT_KEY_WONDER_NEWGRANGE_HELP', '';
------------------------------------------------------------------------------------------------------------------------------------------
...
CREATE TRIGGER IF NOT EXISTS JarCatalhoyuk1 AFTER INSERT ON Resources
WHEN NEW.AnimalResource=1 AND NEW.LandResource=1 AND NEW.ResourceClassType!='RESOURCECLASS_LUXURY'
BEGIN
	INSERT OR REPLACE INTO Building_LocalResourceOrs 
					(BuildingType,			ResourceType)
	SELECT DISTINCT	'BUILDING_CATALHOYUK',	 NEW.Type;
	
	INSERT OR REPLACE INTO Building_ResourceYieldChangesGlobal 
					(BuildingType,			ResourceType,	YieldType,		Yield)
	SELECT DISTINCT	'BUILDING_CATALHOYUK',	 NEW.Type,		'YIELD_FAITH',	1;
END;

CREATE TRIGGER IF NOT EXISTS JarCatalhoyuk2 AFTER UPDATE ON Resources
WHEN NEW.AnimalResource=1 AND NEW.LandResource=1 AND NEW.ResourceClassType!='RESOURCECLASS_LUXURY'
BEGIN
	INSERT OR REPLACE INTO Building_LocalResourceOrs 
					(BuildingType,			ResourceType)
...
INSERT OR REPLACE INTO Language_en_US 
		(Tag, 							 Text)
SELECT	'TXT_KEY_WONDER_CATALHOYUK_HELP', '';
------------------------------------------------------------------------------------------------------------------------------------------
-- LIANGZHU
------------------------------------------------------------------------------------------------------------------------------------------
...
INSERT OR REPLACE INTO Language_en_US 
		(Tag, 							 Text)
SELECT	'TXT_KEY_WONDER_LIANGZHU_HELP', 'Places 2 new [ICON_RES_JADE] Jade resources inside City range if possible.';
------------------------------------------------------------------------------------------------------------------------------------------
-- GOEBEKLI TEPE
------------------------------------------------------------------------------------------------------------------------------------------
