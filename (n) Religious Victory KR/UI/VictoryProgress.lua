...
function PopulateReligious()
	...
	if(religiousVictory ~= nil and bVictoryEnabled) then
		...
		for iPlayerLoop = 0, GameDefines.MAX_CIV_PLAYERS-1, 1 do
			...
			if(not pPlayer:IsMinorCiv() and pPlayer:IsEverAlive() and eReligion ~= -1) then
				...
				local iPlayer = pPlayer:GetID();
				print("eReligion and Religion")
				print(pPlayer:GetNameKey())
				print(eReligion)
				local pOGPlayer = pPlayer
				...
				local victoryPercent = spread/100;
				...
				item.IconFrameGlow:SetHide(spread < iDenominator);

				local txt = Locale.Lookup(statusKey, religion.IconString, religion.Description, iSpread, iDenominator, pOGPlayer:GetNameKey());
				item.IconFrame:SetToolTipString(txt);
