...
local Decisions_TaxReform = {}
	...
	function(pPlayer)		
		...
		local iCost = math.ceil(pPlayer:GetTotalPopulation() * 5 * iMod * ((Game.GetGameTurn()/100)+1))
		local iReward = math.ceil(pPlayer:GetTotalPopulation() * 10 * iMod * ((Game.GetGameTurn()/100)+1))
...
local Decisions_WeightsFormal = {}
	...
	function(pPlayer)
		...
		for pCity in pPlayer:Cities() do
			pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_WEIGHTSFORMAL, 1, true)
		end
...
local Decisions_InviteGreatPerson = {}
	...
	function(pPlayer)
		Decisions_InviteGreatPerson.Data2 = nil
		Decisions_InviteGreatPerson.Data3 = nil
		Decisions_InviteGreatPerson.Data4 = nil
		...
		if #tAdvisors < 1 then return false, false end
		...
		if not pTargetCity then return false, false end
		Decisions_InviteGreatPerson.Data2 = tGreatPerson
		Decisions_InviteGreatPerson.Data3 = math.ceil(25 * pTargetCity:GetSpecialistUpgradeThreshold(iUnitClass) * 0.01) * 100
		Decisions_InviteGreatPerson.Data4 = iSpecialist
		if iCurrentEra < GameInfoTypes.ERA_CLASSICAL then return false, false end
		if pPlayer:GetNumCities() < 1 then
			return false, false
		end
		
		local iCost = math.ceil((iCurrentEra + 1) * 150 * iMod * ((Game.GetGameTurn()/100)+1))
	...
	function(pPlayer)
		...
		local iCost = math.ceil((iCurrentEra + 1) * 150 * iMod * ((Game.GetGameTurn()/100)+1))
		pPlayer:ChangeGold(-iCost)
		local tGreatPerson = Decisions_InviteGreatPerson.Data2
		local iPoints = Decisions_InviteGreatPerson.Data3
		local iSpecialist = Decisions_InviteGreatPerson.Data4
		for pCity in pPlayer:Cities() do
			if pCity:GetSpecialistGreatPersonProgressTimes100(iSpecialist) > 0 then
				pCity:ChangeSpecialistGreatPersonProgressTimes100(iSpecialist, iPoints+50)
			end
		end
...
local Decisions_PromoteTheArts = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((iCurrentEra + 1) * 275 * iMod * ((Game.GetGameTurn()/100)+1))
...
local Decisions_CodeOfLaws = {}
	...
	function(pPlayer)
		if load(pPlayer, "Decisions_CodeOfLaws") == true then
			if pPlayer:GetCapitalCity():GetNumBuilding(GameInfoTypes.BUILDING_DECISIONS_CODEOFLAWS) == 0 then
				save(pPlayer, "Decisions_CodeOfLaws", false)
				return true, true
			end
	...
	function(pPlayer)
		...
		pPlayer:ChangeNumResourceTotal(iMagistrate, -1)
		pPlayer:ChangeJONSCulture(-iCost)
		pPlayer:GetCapitalCity():SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_CODEOFLAWS, 1, true)
...
local Decisions_NationaliseUtilities = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((150*iMod)+(150*(Game.GetGameTurn()/100)))
		local iTurns = math.ceil(2*iGAMod)
	...
	function(pPlayer)
		...
		pPlayer:ChangeNumResourceTotal(iMagistrate, -1)
		pPlayer:ChangeGold(-iCost)
		pPlayer:GetCapitalCity():SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_NATIONALISEUTILITIES, 1, true)
...
local Decisions_PublicTransport = {}
	...
	function(pPlayer)
		--Generate Decisions
		if not(Teams[pPlayer:GetTeam()]:IsHasTech(GameInfoTypes.TECH_RADIO)) then return end
...
local Decisions_UniversityGrants = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((525*iMod)+(525*(Game.GetGameTurn()/100)))
	...
	function(pPlayer)
		...
		pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_UNIVERSITYGRANTS, 1, true)
...
local Decisions_SwordsAndShields = {}
	Decisions_SwordsAndShields.Name = "TXT_KEY_DECISIONS_SWORDSANDSHIELDS"
	Decisions_SwordsAndShields.Desc = "TXT_KEY_DECISIONS_SWORDSANDSHIELDS_DESC"
	Decisions_SwordsAndShields.Weight = 0
	Decisions_SwordsAndShields.CanFunc = (
	function(pPlayer)
		local iEra = load(pPlayer, "Decisions_SwordsAndShields")
		local iCurrentEra = pPlayer:GetCurrentEra()
		if iEra ~= nil then
			if iEra < iCurrentEra then
				save(pPlayer, "Decisions_SwordsAndShields", nil)
			else
				Decisions_SwordsAndShields.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_SWORDSANDSHIELDS_ENACTED_DESC")
				return false, false, true
			end
		end
		if pPlayer:GetNumCities() < 1 then
		...
		if load(pPlayer, "Decisions_SwordsAndShields") == true then
			Decisions_SwordsAndShields.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_SWORDSANDSHIELDS_ENACTED_DESC")
			return false, false, true
		end
		Decisions_SwordsAndShields.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_SWORDSANDSHIELDS_DESC")
		if (pPlayer:GetNumResourceAvailable(iMagistrate, false) < 1) then return true, false end
	...
	Decisions_SwordsAndShields.DoFunc = (
	function(pPlayer)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -1)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_SWORDSANDSHIELDS_CHOICE_1)
		save(pPlayer, "Decisions_SwordsAndShields", pPlayer:GetCurrentEra())
	end
	)
	
tDecisions.Decisions_SwordsAndShields = Decisions_SwordsAndShields

--------------------------------------------------------------------------------------------------------------------------
-- Establish a Moral Alignment
----------------------------------------------------------------------------------------------------------------------------
local Decisions_Pragmatic = {}
	Decisions_Pragmatic.Name = "TXT_KEY_DECISIONS_PRAGMATIC"
	Decisions_Pragmatic.Desc = "TXT_KEY_DECISIONS_PRAGMATIC_DESC"
	Decisions_Pragmatic.Weight = 0
	Decisions_Pragmatic.CanFunc = (
	function(pPlayer)
		if pPlayer:GetNumResourceAvailable(iMorality, false) >= 60 then
			Decisions_Pragmatic.Name = "TXT_KEY_BUILDING_BENEVOLENCE"
			Decisions_Pragmatic.Desc = Locale.ConvertTextKey("TXT_KEY_BUILDING_BENEVOLENCE_HELP")
			return false, false, true
		elseif pPlayer:GetNumResourceAvailable(iMorality, false) <= 40 then
			Decisions_Pragmatic.Name = "TXT_KEY_BUILDING_MALEVOLENCE"
			Decisions_Pragmatic.Desc = Locale.ConvertTextKey("TXT_KEY_BUILDING_MALEVOLENCE_HELP")
			return false, false, true
		elseif pPlayer:GetNumResourceAvailable(iMorality, false) > 40 and pPlayer:GetNumResourceAvailable(iMorality, false) < 60 then
			Decisions_Pragmatic.Name = "TXT_KEY_BUILDING_PRAGMATIC"
			Decisions_Pragmatic.Desc = Locale.ConvertTextKey("TXT_KEY_BUILDING_PRAGMATIC_HELP")
			return false, false, true
		end
	end
	)
	Decisions_Pragmatic.DoFunc = (
	function(pPlayer)
	end
	)
	
tDecisions.Decisions_Pragmatic = Decisions_Pragmatic