...
local Decisions_ThaiAlphabet = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(250 * iMod * ((Game.GetGameTurn()/100)+1))
	...
	function(pPlayer)
		...
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_THAIALPHABET_CHOICE_1)
...
local Decisions_SiamEmbassies = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(#Decisions_SiamEmbassies.Data1 * 75 * iMod * ((Game.GetGameTurn()/100)+1))
...
local Decisions_VeniceStMark = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((500*iMod)+(500*(Game.GetGameTurn()/100)))
...
local Decisions_VenetianArsenale = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((450*iMod)+(450*(Game.GetGameTurn()/100)))
	...
	function(pPlayer)
		...
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_VENETIANARSENALE_CHOICE_1)
...
local Decisions_PolandSejm = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(300*iMod*((Game.GetGameTurn()/100)))
	...
	function(pPlayer)
		...
		pPlayer:ChangeJONSCulture(-iCost)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		pPlayer:GetCapitalCity():SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_GREATSEJM, 1, true)
...
local Decisions_PolandUniversity = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((pCity:GetBuildingPurchaseCost(GameInfoTypes.BUILDING_UNIVERSITY)) * 1.2 * ((Game.GetGameTurn()/100)+1))
...
local Decisions_IncanQuipucamayocs = {}
	...
	function(pPlayer)
		...
		local iReward = math.ceil(iMod * 40 * (pPlayer:GetImprovementCount(GameInfoTypes.IMPROVEMENT_TERRACE_FARM) + pPlayer:GetImprovementCount(GameInfoTypes.IMPROVEMENT_FARM)) * ((Game.GetGameTurn()/100)+1))
...
local Decisions_IncanChasquis = {}
	...
	function(pPlayer)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -1)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_INCANCHASQUIS_CHOICE_1)
...
local Decisions_SpainColumbus = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((300*iMod)+(300*(Game.GetGameTurn()/100)))
	...
	function(pPlayer)
		local iCost = math.ceil((300*iMod)+(300*(Game.GetGameTurn()/100)))
		local tCaravels = InitUnitFromCity(pPlayer:GetCapitalCity(), GameInfoTypes.UNIT_SPANISH_CONQUISTADOR, 3)
		tCaravels[1]:SetName("Hernando Pizarro")
		tCaravels[2]:SetName("Gonzalo Pizarro")
		tCaravels[3]:SetName("Pinta")
		local tAdmiral = InitUnitFromCity(pPlayer:GetCapitalCity(), GameInfoTypes.UNIT_GREAT_GENERAL, 1)
		tAdmiral[1]:SetName("Francisco Pizarro")
		pPlayer:ChangeGold(-iCost)
...
local Decisions_SpanishInquisition = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((200*iMod)+(200*(Game.GetGameTurn()/100)))
		Decisions_SpanishInquisition.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_SPANISHINQUISITION_DESC", iCost)

		if (Teams[pPlayer:GetTeam()]:IsHasTech(GameInfoTypes.TECH_ASTRONOMY)) and (pPlayer:GetGold() >= iCost) and (pTargetCity ~= nil) then
			return true, true
		else
			return true, false
		end
	...
	function(pPlayer)
		...
		local tCaravels = InitUnitFromCity(pTargetCity, GameInfoTypes.UNIT_CARAVEL, 2)
		tCaravels[1]:SetName("Pinta")
		tCaravels[2]:SetName("Niña")
		local tAdmiral = InitUnitFromCity(pTargetCity, GameInfoTypes.UNIT_GREAT_ADMIRAL, 1)
...
local Decisions_DutchWisselbank = {}
	...
	function(pPlayer)
		...
		local iCostC = math.ceil((50*iMod)+(50*(Game.GetGameTurn()/100)))
		local iCostG = math.ceil((75*iMod)+(75*(Game.GetGameTurn()/100)))
	...
	function(pPlayer)
		...
		pPlayer:GetCapitalCity():SetNumRealBuilding(GameInfoTypes.BUILDING_BANK, 1, true)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_DUTCHWISSELBANK_CHOICE_1)
...
local Decisions_DutchNavy = {}
	...
	function(pPlayer)
		...
		pPlayer:InitUnit(GameInfoTypes.UNIT_CARGO_SHIP, pPlot:GetX(), pPlot:GetY())
		pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_DUTCHNAVY, 1, true)
...
local Decisions_GermanConfederation = {}
	...
	function(pPlayer)
		...
		local iCostC = math.ceil(pPlayer:GetNextPolicyCost() * 0.45)
		local iCostG = math.ceil(600 * iMod * ((Game.GetGameTurn()/100)+1))
	...
	function(iPlayer)
		...
		if pPlayer:CountNumBuildings(GameInfoTypes.BUILDING_DECISIONS_GERMANCONFEDERATION) > 0 then
			...
			pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_GERMANCONFEDERATION2, iNumCS, true)
		end
	end
	)
...
local Decisions_RomanConcrete = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(pPlayer:GetResearchCost(iTech) * 0.35 * ((Game.GetGameTurn()/125)+1))
		...
		if (Teams[pPlayer:GetTeam()]:IsHasTech(iTech)) and (pPlayer:GetGold() >= iCost) then
			return true, true
		else
			return true, false
		end
...
local Decisions_RomanPanem = {}
	...
	function(pPlayer)
		...
		if load(pPlayer, "Decisions_RomanPanem") == true then
			Decisions_RomanPanem.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_ROMANPANEM_ENACTED_DESC")
			return false, false, true
		end
		local iTech = GameInfoTypes.TECH_IRON_WORKING
		local iCost = math.ceil(65 * iMod * ((Game.GetGameTurn()/100)+1))
		Decisions_RomanPanem.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_ROMANPANEM_DESC", iCost)
		if (pPlayer:GetNumResourceAvailable(iMagistrate, false) < 1) then return true, false end
		

		if (pPlayer:GetGold() >= iCost) and (pPlayer:GetNumCities() > 0) and (Teams[pPlayer:GetTeam()]:IsHasTech(iTech)) then
			return true, true
		else
			return true, false
		end
	...
	function(pPlayer)
		...
		local iCost = math.ceil(65 * iMod * ((Game.GetGameTurn()/100)+1))
		pPlayer:ChangeGold(-iCost)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_ROMEBALLISTA_CHOICE_1)
		save(pPlayer, "Decisions_RomanPanem", true)
	end
	)
...
local Decisions_PolynesiaPaddle = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(75 * iMod * ((Game.GetGameTurn()/100)+1))
		...
		iCost = math.ceil(15 * iNumUnits * iMod * ((Game.GetGameTurn()/100)+1))
	...
	function(pPlayer)
		...
		local iCost = math.ceil(15 * iNumUnits * iMod * ((Game.GetGameTurn()/100)+1))
...
local Decisions_PolynesiaRunanga = {}
	...
	function(pPlayer)
		...
		local iEra = pPlayer:GetCurrentEra()
		if (iEra == GameInfoTypes.ERA_CLASSICAL or iEra == GameInfoTypes.ERA_MEDIEVAL) and (pPlayer:GetNumResourceAvailable(iMagistrate, false) > 1) then 
			return true, true
		else
			return true, false
		end
...
local Decisions_EgyptianNome = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(5 * 37 * iMod * ((Game.GetGameTurn()/100)+1))
		local iReward = math.ceil(10 * 37 * iMod * ((Game.GetGameTurn()/100)+1))
	...
	--	if pPlayer:GetCivilizationType() ~= GameInfoTypes.CIVILIZATION_EGYPT then return false, false end
	--	
	--	local iReward = math.ceil((pPlayer:GetCurrentEra() + 1) * iMod * 75 * ((Game.GetGameTurn()/100)+1))
	--	
		...
		if load(pPlayer, "Decisions_EgyptValleyOfTheKings") == true then
			for pCity in pPlayer:Cities() do
				...
				if Decisions_EgyptValleyOfTheKings.Wonders[iProduction] then
					pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_EGYPTVALLEYOFTHEKINGS_F, Decisions_EgyptValleyOfTheKings.Wonders[iProduction].Faith, true)
					pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_EGYPTVALLEYOFTHEKINGS_C, Decisions_EgyptValleyOfTheKings.Wonders[iProduction].Culture, true)
				else
...
local Decisions_SwedenIndelningsverket = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((375*iMod)+(375*(Game.GetGameTurn()/100)))
		Decisions_SwedenIndelningsverket.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_SWEDENINDELNINGSVERKET_DESC", iCost)
		
		if (pPlayer:GetNumResourceAvailable(iMagistrate, false) < 2) then return true, false end
		if (Teams[pPlayer:GetTeam()]:IsHasTech(GameInfoTypes.TECH_RIFLING)) and (Teams[pPlayer:GetTeam()]:IsHasTech(GameInfoTypes.TECH_METALLURGY)) and (pPlayer:GetGold() >= iCost) and (pPlayer:GetCapitalCity() ~= nil) then
			return true, true
		else
			return true, false
		end
...
local Decisions_SwedishRiksdag = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(350 * iMod * ((Game.GetGameTurn()/100)+1))
...
local Decisions_RussianStreltsy = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(400 * iMod * ((Game.GetGameTurn()/100)+1))
		...
		for pCity in pPlayer:Cities() do
			if pCity:IsHasBuilding(GameInfoTypes.BUILDING_CONSTABLE) or pCity:IsCapital() then
				...
				function(pPlayer, pCity)
					...
					local iReward = math.ceil(iPop * 20 * iMod * ((Game.GetGameTurn()/100)+1))
...
local Decisions_ShoshoneComanche = {}
	...
	function(pPlayer)
		...
		Decisions_ShoshoneComanche.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_SHOSHONECOMANCHE_DESC", iReward * ((Game.GetGameTurn()/100)+1))
		
		if (pPlayer:GetNumResourceAvailable(iMagistrate, false) < 2) then return true, false end

		if (Teams[pPlayer:GetTeam()]:IsHasTech(GameInfoTypes.TECH_MILITARY_SCIENCE)) and (pPlayer:GetCapitalCity() ~= nil) then 
			return true, true
		else 
			return true, false
		end
	...
	function(pPlayer)
		local iReward = math.ceil((pPlayer:GetNumResourceAvailable(GameInfoTypes.RESOURCE_HORSE, false) + 5) * 30 * iMod * ((Game.GetGameTurn()/100)+1))
...
local Decisions_ShoshoneWilderness = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((15*iMod)+(15*(Game.GetGameTurn()/100)))
...
local Decisions_IroquoisLawofPeace = {}
	...
	function(pPlayer)
		...
		--if not(bConnected) then return true, false end
		--return true, true
		if (iCities >= 3 and bConnected == true) or (pPlayer:GetNumResourceAvailable(iMagistrate, false) >= 4) then
			return true, true
		else
			return true, false
		end
	...
	function(pPlayer)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		pPlayer:GetCapitalCity():SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_IROQUOISLAWOFPEACE, 1, true)
	...
	function(iPlayer)
		local pPlayer = Players[iPlayer]
		if (pPlayer:GetCivilizationType() ~= GameInfoTypes.CIVILIZATION_IROQUOIS) then return end
		if load(pPlayer, "Decisions_IroquoisLawofPeace") == true then
			pPlayer:GetCapitalCity():SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_IROQUOISLAWOFPEACE, 1, true)
		end
	end
...
local Decisions_ByzantiumWalls = {}
	...
	function(pPlayer)
		...
		local pCity = pPlayer:GetCapitalCity()
		if not(pCity) then return false, false end
		local iCost = math.ceil(pCity:GetBuildingPurchaseCost(GameInfoTypes.BUILDING_WALLS) * 1.2 * ((Game.GetGameTurn()/100)+1))
	...
	function(pPlayer)
		local iCost = math.ceil((pPlayer:GetCapitalCity():GetBuildingPurchaseCost(GameInfoTypes.BUILDING_WALLS)) * 1.2 * ((Game.GetGameTurn()/100)+1))
...
local Decisions_ByzantiumPorphyrogennetos = {}
	...
	function(pPlayer)
		...
		--local iCost = math.ceil(300 * iMod * ((Game.GetGameTurn()/100)+1))
		local iCost = math.ceil(5 * 300 * iMod * ((Game.GetGameTurn()/100)+1))
		local iReward = math.ceil(10 * 300 * iMod * ((Game.GetGameTurn()/100)+1))
...
local Decisions_IndonesiaSpice = {}
	...
	function(pPlayer)
		...
		pPlayer:InitUnit(GameInfoTypes.UNIT_CARGO_SHIP, pPlot:GetX(), pPlot:GetY())
		pPlayer:InitUnit(GameInfoTypes.UNIT_CARGO_SHIP, pPlot:GetX(), pPlot:GetY())
		pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_INDONESIASPICE, 1, true)
...
local Decisions_AmericaEmancipation = {}
	...
	function(pPlayer)
		...
		if (iEra == GameInfoTypes.ERA_INDUSTRIAL or iEra == GameInfoTypes.ERA_MODERN) and (pPlayer:GetNumResourceAvailable(iMagistrate, false) > 1) then 
			return true, true
		else
			return true, false
		end
...
local Decisions_AmericanPurchase = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(iMod * 600 * ((Game.GetGameTurn()/100)+1))
...
local Decisions_ArabiaHouseOfWisdom = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(iMod * 500 * ((Game.GetGameTurn()/100)+1))
		local iNumTrade = 0
		for pUnit in pPlayer:Units() do if pUnit:IsTrade() then iNumTrade = iNumTrade + 1 end end
		local iReward = math.ceil(25 * iMod * ((Game.GetGameTurn()/100)+1)) * iNumTrade
	...
	function(pPlayer)
		...
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_ARABIAHOUSEOFWISDOM_CHOICE_1)
...
local Decisions_FranceArmee = {}
	...
	function(pPlayer)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		--pPlayer:GetCapitalCity():SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_FRANCEARMEE, 1)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_FRANCEARMEE_CHOICE_1)
...
local Decisions_FranceEstates = {}
	...
	function(pPlayer)
		...
		pPlayer:ChangeJONSCulture(-iCost)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		pPlayer:GetCapitalCity():SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_ESTATES_GENERAL, 1)
...
local Decisions_OttomanKanuni = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((225*iMod)+(225*(Game.GetGameTurn()/100)+1))
	...
	function(pPlayer)
		...
		pPlayer:ChangeJONSCulture(-iCost)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_OTTOMANKANUNI_CHOICE_1)
...
local Decisions_OttomanGunpowder = {}
	...
	function(pPlayer)
		...
		if (pPlayer:GetGold() < iCost) then	return true, false end
		if (pPlayer:GetNumResourceAvailable(iMagistrate, false) < 2) then return true, false end
		if not(Teams[pPlayer:GetTeam()]:IsHasTech(GameInfoTypes.TECH_METALLURGY)) then return true, false end
	...
	function(pPlayer)
		...
		pPlayer:ChangeGold(-iCost)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_OTTOMANGUNPOWDER_CHOICE_1)
...
local Decisions_EnglishVirginia = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((600*iMod)+(600*(Game.GetGameTurn()/100)))
...
local Decisions_EnglishArmada = {}
	...
	function(pPlayer)
		...
		if not(Teams[pPlayer:GetTeam()]:IsHasTech(GameInfoTypes.TECH_MACHINERY)) then return true, false end
		if (pPlayer:GetCapitalCity() == nil) then return true, false end
		if (pPlayer:GetNumResourceAvailable(iMagistrate, false) < 2) then return true, false end
		return true, true
	end
	...
	function(pPlayer)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_ENGLISHARMADA_CHOICE_1)
...
local Decisions_DenmarkThing = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((225*iMod)+(225*(Game.GetGameTurn()/100)))
...
local Decisions_CarthageHanno = {}
	...
	function(pPlayer, tUnitTypes)		
		...
		if load(pPlayer, "Decisions_CarthageHanno") == true then
	...
	function(pPlayer, tUnitTypes, iUnitType)
		...
		save(pPlayer, "Decisions_CarthageHanno", true)
	end
	)
...
local Decisions_CarthageCothons = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(80 * iMod * ((Game.GetGameTurn()/100)+1))
		if (pPlayer:GetNumCities() > 4) then iCost = math.ceil(20 * iMod * pPlayer:GetNumCities() * ((Game.GetGameTurn()/100)+1)) end
	...
	function(pPlayer)
		local iCost = math.ceil(20 * iMod * pPlayer:GetNumCities() * ((Game.GetGameTurn()/100)+1))
		pPlayer:ChangeGold(-iCost)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -1)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_CARTHAGECOTHONS_CHOICE_1)
...
local Decisions_MayaBallGame = {}
	...
	function(pPlayer)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_MAYABALLGAME_CHOICE_1)
...
local Decisions_MayaAtlatl = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(60 * iMod * iNum * ((Game.GetGameTurn()/100)+1))
	...
	function(pPlayer)
		...
		for iKey, pUnit in ipairs(Decisions_MayaAtlatl.tAtlatl) do
			local pNewUnit = pPlayer:InitUnit(GameInfoTypes.UNIT_CROSSBOWMAN, pUnit:GetX(), pUnit:GetY())
...
local Decisions_KoreanHangul = {}
	...
	function(pPlayer)
		...
		local iEra = pPlayer:GetCurrentEra()
		--if not(iEra == GameInfoTypes.ERA_RENAISSANCE or iEra == GameInfoTypes.ERA_MEDIEVAL) then return true, false end
		if (iEra == GameInfoTypes.ERA_MEDIEVAL or iEra == GameInfoTypes.ERA_RENAISSANCE) and (pPlayer:GetNumResourceAvailable(iMagistrate, false) >= 2) then 
			return true, true
		else
			return true, false
		end
	...
	function(pPlayer)
		...
		--pPlayer:ChangeGold(-iCost)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_KOREANHANGUL_CHOICE_1)
...
local Decisions_KoreanGunpowder = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((400*iMod)+(400*(Game.GetGameTurn()/100)))
	...
	function(pPlayer)
		local iCost = math.ceil((400*iMod)+(400*(Game.GetGameTurn()/100)))
		pPlayer:ChangeGold(-iCost)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_KOREANGUNPOWDER_CHOICE_1)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
...
local Decisions_SonghaiSankore = {}
	...
	function(pPlayer)
		...
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_SONGHAISANKORE_CHOICE_1)
...
local Decisions_SonghaiTradeRoute = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(150 * iMod * ((Game.GetGameTurn()/100)+1)) 		
...
local Decisions_PortugalCasaDaIndia = {}
	...
	function(pPlayer)
		...
		pPlayer:ChangeGold(-iCost)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -1)
		pPlayer:GetCapitalCity():SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_PORTUGALCASADAINDIA, 1, true)
...
local Decisions_PortugalTradeTaxes = {}
	...
	function(pPlayer)
		...
		local iReward = math.ceil(iNumUnits * iMod * 300 * ((Game.GetGameTurn()/100)+1))
...
local Decisions_AssyriaResettlement = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil((325*iMod)+(325*(Game.GetGameTurn()/100)))
...
local Decisions_AustrianHofkriegsrat = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(450 * iMod * ((Game.GetGameTurn()/100)+1))
	...
	function(pPlayer)
		...
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_AUSTRIANHOFKRIEGSRAT_CHOICE_1)
	end
	)
...
local Decisions_AustrianHabsburg = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(175 * iMod * ((Game.GetGameTurn()/100)+1))
	...
	function(pPlayer)
		...
		pPlayer:GetCapitalCity():SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_AUSTRIANHABSBURG, 1)
		for pCity in pPlayer:Cities() do
			pCity:ChangeWeLoveTheKingDayCounter(iTurns)
		end
	end
...
local Decisions_BrazilEthanol = {}
	...
	function(pPlayer)
		...
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_BRAZILETHANOL_CHOICE_1)
...
local Decisions_BrazilBandeirantes = {}
	...
	function(pPlayer)
		...
		local iCult = math.ceil(75 * iMod * ((Game.GetGameTurn()/100)+1))
		local iGold = math.ceil(400 * iMod * ((Game.GetGameTurn()/100)+1))
	...
	function(pPlayer)
		...
		pPlayer:ChangeGold(-iGold)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_BRAZILBANDEIRANTES_CHOICE_1)
...
local Decisions_ChinaImperialExam = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(325 * iMod)
	...
	function(pPlayer)
		...
		pPlayer:ChangeGold(-iCost)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_CHINAIMPERIALEXAM_CHOICE_1)
...
local Decisions_ChinaFubingZhi = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(325 * iMod * ((Game.GetGameTurn()/100)+1))
...
local Decisions_PersiaSatrapy = {}
	...
	Decisions_PersiaSatrapy.Monitors = {}
	Decisions_PersiaSatrapy.Monitors[GameEvents.PlayerDoTurn] =  (
	function(iPlayer)
		local pPlayer = Players[iPlayer]
		if (pPlayer:GetCivilizationType() ~= GameInfoTypes.CIVILIZATION_PERSIA) then return end
		if not(pPlayer:GetCapitalCity()) then return end
		if load(pPlayer, "Decisions_PersiaSatrapy") == true then
			for pCity in pPlayer:Cities() do		
				if pCity:IsHasBuilding(GameInfoTypes.BUILDING_SATRAPS_COURT) then
					pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_PERSIASATRAPY, 1, true)
				else
					pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_PERSIASATRAPY, 0)
				end
			end
		end
	end
...
local Decisions_PersiaSavaran = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(60 * iMod * ((Game.GetGameTurn()/100)+1))
	...
	function(pPlayer)
		...
		pPlayer:ChangeGold(-iCost)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -1)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_PERSIASAVARAN_CHOICE_1)
...
local Decisions_MoroccoBlackGuard = {}
	...
	function(pPlayer)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -1)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_MOROCCOBLACKGUARD_CHOICE_1)
...
local Decisions_MongoliaExecution = {}
	...
	function(pPlayer)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_MONGOLIAEXECUTION_CHOICE_1)
...
local Decisions_MongoliaSilkRoad = {}
	...
	function(pPlayer)
		...
		pPlayer:ChangeJONSCulture(-iCost)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -1)
		pPlayer:GetCapitalCity():SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_MONGOLIASILKROAD, 1, true)
	...
	function(iPlayer)
		local pPlayer = Players[iPlayer]
		if (pPlayer:GetCivilizationType() ~= GameInfoTypes.CIVILIZATION_MONGOL) then return end
		if load(pPlayer, "Decisions_MongoliaSilkRoad") == true then
			pPlayer:GetCapitalCity():SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_MONGOLIASILKROAD, 1, true)
			for pCity in pPlayer:Cities() do
				pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_MONGOLIASILKROAD2, 1, true)
			end
		end
	end
	)
...
local Decisions_BabylonObservatory = {}
	...
	function(pPlayer)
		...
		local pCity = pPlayer:GetCapitalCity()
		if not(pCity) then return false, false end
		local iCost = math.ceil((pCity:GetBuildingPurchaseCost(GameInfoTypes.BUILDING_OBSERVATORY)))
...
local Decisions_BabylonGates = {}
	...
	function(pPlayer)
		...
		local iReward = math.ceil(60 * iMod * ((Game.GetGameTurn()/100)+1))
...
local Decisions_CeltsTinTrade = {}
	...
	function(pPlayer)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		for pCity in pPlayer:Cities() do
			pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_CELTSTINTRADE, 1, true)
		end
	...
	function(iPlayer)
		local pPlayer = Players[iPlayer]
		if (pPlayer:GetCivilizationType() ~= GameInfoTypes.CIVILIZATION_CELTS) then return end
		if load(pPlayer, "Decisions_CeltsTinTrade") == true then
			for pCity in pPlayer:Cities() do
				pCity:SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_CELTSTINTRADE, 1, true)
			end
...
local Decisions_CeltsUilleannPipes = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(600 * iMod * ((Game.GetGameTurn()/100)+1))
		local iReward = math.ceil((pPlayer:CountNumBuildings(GameInfoTypes.BUILDING_AMPHITHEATER) + pPlayer:CountNumBuildings(GameInfoTypes.BUILDING_CEILIDH_HALL)) * 40 * iMod * ((Game.GetGameTurn()/100)+1))
...
local Decisions_GreecePhalanx = {}
	...
	function(pPlayer)
		...
		Decisions_GreecePhalanx.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_GREECEPHALANX_DESC")
		if (pPlayer:GetNumResourceAvailable(iMagistrate, false) < 1) then return true, false end
		if not(Teams[pPlayer:GetTeam()]:IsHasTech(GameInfoTypes.TECH_ARCHERY)) then return true, false end
		return true, true
	end
	)
...
local Decisions_IndiaReligions = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(50 * iMod * ((Game.GetGameTurn()/100)+1))
		Decisions_IndiaReligions.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_INDIARELIGIONS_DESC", iCost)		
		
		if (pPlayer:GetFaith() < iCost) then return true, false end
		if (pPlayer:GetNumResourceAvailable(iMagistrate, false) < 1) then return true, false end
		if (GetNumReligionsinEmpire(pPlayer) < 1)  then return true, false end
	...
	function(pPlayer)
		...
		pPlayer:ChangeFaith(-iCost)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -1)
		pPlayer:GetCapitalCity():SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_INDIARELIGIONS, 1, true)
...
local Decisions_IndiaDevangariScript = {}
	...
	function(pPlayer)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_INDIADEVANGARISCRIPT_CHOICE_1)
...
local Decisions_JapanShogunate = {}
	...
	function(pPlayer)
		...
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_JAPANSHOGUNATE_CHOICE_1)
...
local Decisions_JapanTheatre = {}
	...
	function(pPlayer)
		...
		pPlayer:ChangeGold(-iCost)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -2)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_JAPANTHEATRE_CHOICE_1)
...
local Decisions_EthiopiaCoinage = {}
	...
	function(pPlayer)
		...
		local iCost = math.ceil(120 * iMod * ((Game.GetGameTurn()/100)+1))
	...
	function(pPlayer)
		...
		pPlayer:ChangeJONSCulture(-iCost)
		pPlayer:ChangeNumResourceTotal(iMagistrate, -1)
		pPlayer:GetCapitalCity():SetNumRealBuilding(GameInfoTypes.BUILDING_DECISIONS_ETHIOPIACOINAGE, 1)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_ETHIOPIACOINAGE_CHOICE_1)
...
local Decisions_EthiopiaSolomonic = {}
	...
	function(iPlayer)
		local pPlayer = Players[iPlayer]
		if (pPlayer:GetCivilizationType() ~= GameInfoTypes.CIVILIZATION_ETHIOPIA) then return end
		if load(pPlayer, "Decisions_EthiopiaSolomonic") == true then
			for pCity in pPlayer:Cities() do
			...
			if iPlayer ~= pPlayer:GetID() then
				...
				if pEnemy:IsEverAlive() and pEnemy:IsAlive() and pTeam:IsHasMet(pEnemy:GetTeam()) then
					...
					function(pPlayer, tData, bMaxEnacted)
						...
						local iReward = math.ceil(120 * iMod * ((Game.GetGameTurn()/100)+1))
						--print(sName, iNumUnits)
						if (iNumUnits ~= nil and iNumUnits > 0) then
							iReward = math.ceil(iNumUnits * 75 * iMod * ((Game.GetGameTurn()/100)+1))
						else 
							iNumUnits = 0
						end
					...
					function(pPlayer, tData)
						...
						local sKey = "Decisions_HunsRansom" .. iEnemy
						local iNumUnits = tData.Units
						local iReward = math.ceil(iNumUnits * 75 * iMod * ((Game.GetGameTurn()/100)+1))
...
local Decisions_HunsRecurveBow = {}
	...
	function(pPlayer, tUnitTypes)
		...
		local iCost = math.ceil(387 * iMod * ((Game.GetGameTurn()/100)+1))
	...
	function(pPlayer, tUnitTypes)
		...
		pPlayer:ChangeNumResourceTotal(iMagistrate, -1)
		pPlayer:ChangeGold(-iCost)
		pPlayer:DoEventChoice(GameInfoTypes.PLAYER_EVENT_DECISIONS_HUNSRECURVEBOW_CHOICE_1)
...
--Something to remove dependency on PolicyType.
local iSipahi = GameInfoTypes.UNIT_DECISIONS_SIPAHI
local iGalleass = GameInfoTypes.UNIT_DECISIONS_GALLEASS
local iBallista = GameInfoTypes.UNIT_DECISIONS_BALLISTA
local iCompanion = GameInfoTypes.UNIT_DECISIONS_CC
local iComanche = GameInfoTypes.UNIT_DECISIONS_COMANCHE
local iLongbow = GameInfoTypes.UNIT_DECISIONS_LONGBOWMAN
local iTurtleShip = GameInfoTypes.UNIT_DECISIONS_TURTLESHIP
local iHakkapeliitta = GameInfoTypes.UNIT_DECISIONS_HAKKAPELIITTA
local iPersia = GameInfoTypes.UNIT_DECISIONS_PERSIASAVARAN

local iSki = GameInfoTypes.UNIT_DECISIONS_SKI
local iPra = GameInfoTypes.UNIT_DECISIONS_PRA

--SurprisinglyHard
local iLancer = GameInfoTypes.UNIT_LANCER
local iGalleasses = GameInfoTypes.UNIT_GALLEASS
local iCavalry = GameInfoTypes.UNIT_CAVALRY
local iCaravel = GameInfoTypes.UNIT_CARAVEL
local iCrossbow = GameInfoTypes.UNIT_CROSSBOWMAN

local iRifle = GameInfoTypes.UNIT_RIFLEMAN
local iInfan = GameInfoTypes.UNIT_INFANTRY

function CanTrainUnitDecisions(iPlayer, iCity, iUnit)
	local pPlayer = Players[iPlayer]
...
function TheLastModder(iPlayer, iOldUnit, iNewUnit, bGoodyHut)
	...
	elseif pUnit:GetUnitType() == iRifle then
		if load(pPlayer, "Decisions_DenmarkThing") == true then
			local pBringerofDeath = pUnit:UpgradeTo(iSki, true)
			pBringerofDeath:Convert(pPlayer:GetUnitByID(iOldUnit), true)
		end
	elseif pUnit:GetUnitType() == iInfan then
		if load(pPlayer, "Decisions_BrazilEthanol") == true then
			local pBringerofDeath = pUnit:UpgradeTo(iPra, true)
			pBringerofDeath:Convert(pPlayer:GetUnitByID(iOldUnit), true)
		end
	end
end

GameEvents.CityCanTrain.Add(CanTrainUnitDecisions)
GameEvents.UnitUpgraded.Add(TheLastModder)