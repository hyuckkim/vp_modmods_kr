...
local doctorTexture = "citizenDoctor.dds"; --MOD
...
function AddBuildingButton( pCity, building )
	...
	if (pCity:IsHasBuilding(buildingID)) and building.IsVisible then --PLAGUE MOD support for compatibility
		...
		if building.SpecialistType then
			...
			elseif building.SpecialistType == "SPECIALIST_SLAVE" then --MOD SUPPORT FOR SLAVERY MOD
				controlTable.BuildingFilledSpecialistSlot1:SetTexture(unemployedTexture);
				controlTable.BuildingFilledSpecialistSlot2:SetTexture(unemployedTexture);
				controlTable.BuildingFilledSpecialistSlot3:SetTexture(unemployedTexture);
...
--		UpdateSpecialists(pCity);
		...
		--PLAGUE MOD
		local iCityHealthIndex = GetCityHealthTotal(pCity);
		...
		if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
			...
			if g_Properties.bFaithSpecialists then
				iFaithPerTurn = iFaithPerTurn + City_GetFaithFromSpecialists(pCity); --MOD
			end
