...
function GetHelpTextForBuilding(iBuildingID, bExcludeName, bExcludeHeader, bNoMaintenance, pCity)
	...
	local iHealthMod = Game.GetBuildingYieldModifier(iBuildingID, GameInfo.Yields["YIELD_HEALTH"].ID);
	if (iHealthMod ~= 0) then
		if iHealthMod > 0 then
			table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_HEALTHMOD", iHealthMod));
		else
			table.insert(lines, Locale.ConvertTextKey("TXT_KEY_PRODUCTION_BUILDING_HEALTHMOD_NEGATIVE", iHealthMod));
		end
	end
	--END MOD
