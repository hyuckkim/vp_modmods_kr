	...
	if( iPlayerID >= 0 ) then
		...
		if (pPlayer:GetNumCities() > 0) then
			...
			if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
				...
				if g_Properties.bFaithSpecialists then
			...
			-- Update Resources
			-----------------------------
			local strResourceText = "[ICON_TRADE]";
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
		...
		if g_Properties.bFaithSpecialists then
			...
			if (iFaithFromSpecialists ~= 0) then
				strText = strText .. "[NEWLINE]";
				strText = strText .. Locale.ConvertTextKey("TXT_KEY_TP_PLAGUE_FAITH_SPECIALISTS", iFaithFromSpecialists);
			end
		end
		--end MOD
...
function HealthTipHandler( control )
	...
	local health = pPlayer:GetEmpireHealthDetails()

	strText = strText .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_OVERALL") .. " " .. ConvertTextColor(health.total);
	strText = strText .. "[NEWLINE]";

	--Trait modifier
	if (health.trait ~= 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(health.trait) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_TRAIT");
	end

	--Policy modifier
	if (health.policy ~= 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(health.policy) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_POLICY");
	end

	--Building modifier
	if (health.building ~= 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(health.building) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_BUILDING");
	end

	--Tech modifier
	if (health.tech ~= 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(health.tech) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_TECH");
	end

	--Trade modifier
	if (health.trade ~= 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(health.trade) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_TRADE");
	end

	--Diversity modifier
	if (health.diversity ~= 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(health.diversity) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_DIVERSITY");
	end

	--Religious modifier
	if (health.religion ~= 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(health.religion) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_RELIGION");
	end

	--Happiness modifier
	if (health.happiness ~= 0) then
		strText = strText .. "[NEWLINE]";
		if health.happiness < 0 then
			strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(health.happiness) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_UNHAPPY");
		else
			strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(health.happiness) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_GOLDENAGE");
		end
	end
