...
### VP의 한국어 번역 모드-모드:  
[(1L) Community Patch INT](https://github.com/LoneGazebo/Community-Patch-DLL)  
커뮤니티 패치의 한국어 모드입니다. 커뮤니티 패치는 새로운 DLL 테이블과 함수, Lua 이벤트, 수많은 버그 픽스, 그리고 향상된 AI를 포함하는 VP의 핵심 모드입니다.  

[(2L) Vox Populi INT](https://github.com/LoneGazebo/Community-Patch-DLL)  
VP(Vox Populi, 민중의 목소리)의 한국어 모드입니다. VP는 유닛, 건물, 사회 정책 등 성능 재조정. 자원 독점, 역사적 사건, 기업 등 추가. 행복도, 국가 불가사의 등 재설계. 게임 메커니즘을 바꿔줍니다.  

[(3aL) VP - EUI Compatibility Files INT](https://github.com/LoneGazebo/Community-Patch-DLL)  
VP용 EUI 호환 파일의 한국어 모드입니다. VP용 EUI 호환 파일 모드는 문명 5의 UI를 개선해주는 [EUI 모드](https://forums.civfanatics.com/threads/enhanced-user-interface.697551/) 를 VP에 호환되게 바꾼 겁니다.  

[(4aL) Squads for VP INT](https://github.com/LoneGazebo/Community-Patch-DLL)  
...
VP에 내장된 이벤트가 12개짜리 데모 버전 같은 맛보기 라면 (7) VP 커뮤니티 이벤트가 200여개나 되는 그 완성본. [종교 이벤트 문제가 있어 패치](https://forums.civfanatics.com/threads/community-patch-events-development.569321/post-16514654) 를 적용했습니다. 한국어 번역은 hyuckkim님의 ChatGPT 번역을 살짝 수정했습니다.  

[UI - Promotion Tree for VP](https://forums.civfanatics.com/threads/ui-promotion-tree-for-vp.690546/)  
게임에서 우측 상단 메뉴에 승급 트리를 보여주는 버튼을 추가해주는 모드입니다. 원본 모드에 한국어를 추가하는 수정을 했습니다. 더 이상 관리하는 사람이 없어 VP에서 분리된 모드로 실제 트리와 맞지 않더라도 고쳐질 계획은 없기에 참고만 하세요.  

### VP용 편의성 모드-모드:  
[(ui) List Enhancements for VP](https://forums.civfanatics.com/threads/list-enhancements-for-vp.638546/) KR  
도시 화면의 유닛 목록을 개선시켜주는 모드입니다.

[(ui) Meaningful Colours (v 16)](https://forums.civfanatics.com/threads/meaningful-colours.636237/)  
...
[Elephant Sound When Circus Built (v 1)](https://forums.civfanatics.com/threads/elephant-sound-when-circus-built.690742/))  
서커스 건설이 완성되었을 때 코끼리 울음소리가 나도록 해주는 단순하지만 직관적인 모드입니다.  
...
[Route Planner for VP (v 1)](https://forums.civfanatics.com/threads/qol-route-planner-more-control-over-automated-route-construction.688850/)  
Route Planner for VP (v 1) INT  
도로 건설 설정을 세세하게 할 수 있게 해주는 모드입니다. [자세한 설명은 이 글](https://gall.dcinside.com/mgallery/board/view/?id=civilization&no=416402)을 참조하세요.  

[UI - Enhanced Rankings (v 13)](https://www.picknmixmods.com/mods/CivV/UI/Enhanced%20Rankings.html)  
UI - Enhanced Rankings INT  
모드 충돌이 나는 InfoAddict 모드를 대체하기 위해 추가한 모드로 문명 순위를 언제든지 볼 수 있게 하는 버튼을 게임 우측 상단에 추가해줍니다. [ㅇㅇ(周年)님이 번역을 먼저](https://gall.dcinside.com/mgallery/board/view/?id=civilization&no=269652) 하셨으나 제가 새로 번역했습니다.  

[UI - Improved City View (Vox Populi EUI) (v 20)](https://forums.civfanatics.com/threads/ui-improved-city-view-vox-populi-with-eui.627067/post-16473637)
...
More Wonder와는 별개의 모드로 함께 사용 가능한 불가사의 추가 모드입니다. 팀타님이 원본을 수정해서 만드셨기에 별도의 [원본 모드](https://forums.civfanatics.com/threads/jarcasts-hidden-wonders-for-vp.698137/)를 요구하지 않습니다.  
...
[(overhaul) Enlightenment Era for VP](https://forums.civfanatics.com/threads/enlightenment-era-for-vp-4-22.698499/)  
(overhaul) Enlightenment Era for VP INT  
많은 이들의 사랑을 받는 계몽 시대 모드로 혹자왈 이 모드가 없으면 VP를 할 이유가 없다고 할 정도로 인기입니다. 역시나 [CAYM님의 번역본](https://cafe.naver.com/civilization4/289556) 이 존재합니다만, 이번에도 참조하되 대부분 새로 번역을 했습니다. 계몽 모드는 다행히 Hokath님이 CAYM님 버전을 기반으로 최신 버전에 맞게 개량해주신 덕에 이를 이용하고 있습니다.  

[(overhaul) More Wonders for VP](https://forums.civfanatics.com/threads/poll-more-wonders-for-vp.653498/)  
(overhaul) More Wonders for VP INT (AI)  
불가사의 추가 모드 중 가장 큰 규모를 자랑하는 모드입니다. 번역 시도한 사람이 두 분 계신데 [구버전을 번역한 분](https://gall.dcinside.com/mgallery/board/view/?id=civilization&no=300915) , [최신 버전 AI 번역 시도한 한 분](https://gall.dcinside.com/mgallery/board/view/?id=civilization&no=417756) 이 있습니다. 그리고 저도 그 많은 양에 식겁해 결국 구글 Gemini Pro 2.5 AI 번역으로 떼웠습니다.  
...
[Civic And Reform (v 22)](https://forums.civfanatics.com/threads/civics-and-reforms.568603/) KR  
정책을 바꿔가며 문명의 방향성을 조정해 지지와 저항, 그리고 외교적 영향을 받게 됩니다.  

[Counter-Reconnaissance (v 44)](https://forums.civfanatics.com/threads/counter-reconnaissance.684481/)  
Counter-Reconnaissance (v 44) INT  
정찰 유닛이 전투에 참여시 이동력이 감소하고 생존 가능성이 높아집니다.  

[Even More Resources for Vox Populi UCS (v 14)](https://forums.civfanatics.com/threads/even-more-resources-for-vox-populi.654431/post-16831556)  
...
[Exploration Heritage (v 46)](https://forums.civfanatics.com/threads/exploration-heritage.662115/)  
Exploration Heritage (v 46) INT  
고레벨 정찰 유닛이 손실되면 경험치 풀이 증가하여 수도와의 거리에 따라 대학에서 승격된 손상되지 않은 정찰 유닛에 턴당 보너스 경험치가 부여됩니다.  

[Gedemon's YnAEMP](https://steamcommunity.com/sharedfiles/filedetails/?id=1623957266) KR  
...
[Maritime Weather+ (v 6)](https://forums.civfanatics.com/threads/maritime-weather.695562/)  
Maritime Weather+ (v 6) INT  
Maritime Battles에서 분리된 모드로 폭풍이라는 움직이는 기상 현상을 추가시켜주는 모드입니다.  

[Really Advanced Setup (v 15)](https://forums.civfanatics.com/threads/really-advanced-setup.486324/) KR  
게임을 시작하기 전의 고급 설정 화면을 훨씬 더 세세하게 바꿔주는 모드입니다.  
※ 번역이 불가한 구간이 있어 번역 진행이 멈췄습니다.  

[Recon Pay (Core0) Terrain Cost (v 7)](https://forums.civfanatics.com/threads/recon-pay-terrain-cost-all-you-can-move-buffet-edition.685224/)  
본래 정찰 유닛의 지형 비용에 대한 조정이 있던 모드라 이름도 저랬으나 취소되고, 초기 이동력 조정과 지형에 대한 반응성이 민감해졌습니다.

[Recon Pay (Core1) Exploration Balancer (v 3)](https://forums.civfanatics.com/threads/recon-pay-terrain-cost-all-you-can-move-buffet-edition.685224/)  
Recon Pay (Core1) Exploration Balancer (v 3) INT  
정찰 유닛에 두 단계로 이루어지는 이동력 밸런스 조정이 추가됩니다.  

[Recon Pay (Recommended1) Fast Coast (v 3)](https://forums.civfanatics.com/threads/recon-pay-terrain-cost-all-you-can-move-buffet-edition.685224/)  
Recon Pay (Recommended1) Fast Coast (v 3) INT  
정찰 유닛이 한 번의 움직임으로 승선하며 연안에서 더 빠르게 이동합니다.  

[Recon Pay (Recommended2) Fast Rivers (v 6)](https://forums.civfanatics.com/threads/recon-pay-terrain-cost-all-you-can-move-buffet-edition.685224/)  
Recon Pay (Recommended2) Fast Rivers (v 6) INT  
정찰 유닛이 강을 따라 더 빠르게 이동합니다.  

[Recon Pay (Recommended3) Fast Coastal (v 7)](https://forums.civfanatics.com/threads/recon-pay-terrain-cost-all-you-can-move-buffet-edition.685224/)  
Recon Pay (Recommended3) Fast Coastal (v 7) INT  
물에 인접한 플롯에 보이지 않는 연안 특징을 추가해 정찰 유닛이 더 빠르게 이동합니다.  

[Recon Pay (Xtra1) Arctician (v 2)](https://forums.civfanatics.com/threads/recon-pay-terrain-cost-all-you-can-move-buffet-edition.685224/)  
Recon Pay (Xtra1) Arctician (v 2) INT  
정찰 유닛이 물리적으로 빙하 위를 건널 수 있습니다.  

[Recon Pay (Xtra2) Free ZoC (v 3)](https://forums.civfanatics.com/threads/recon-pay-terrain-cost-all-you-can-move-buffet-edition.685224/)  
Recon Pay (Xtra2) Free ZoC (v 3) INT  
이동 중에 통제 구역(ZOC)를 무시합니다. Counter-Reconnaissance 모드와 같이 사용할 수 없어 모드팩에서 제외, 모드는 기호에 맞게 둘 중 하나를 골라 쓰시면 됩니다.  

[Recon Pay (Xtra3) Trees Obstruct Mounted Armor (v 6)](https://forums.civfanatics.com/threads/recon-pay-terrain-cost-all-you-can-move-buffet-edition.685224/)  
Recon Pay (Xtra3) Trees Obstruct Mounted Armor (v 6) INT  
숲, 정글, 습지, 오아시스에서 탑승물과 갑옷의 이동 속도를 늦춥니다.  

[Recon Pay (Xtra4) Change Port (v 2)](https://forums.civfanatics.com/threads/recon-pay-terrain-cost-all-you-can-move-buffet-edition.685224/)  
공수부대 이전의 정찰 유닛이 항구를 변경할 수 있습니다. (위대한 제독 처럼)  

[Recon Pay (Xtra5) Horse Thieves (v 1)](https://forums.civfanatics.com/threads/recon-pay-terrain-cost-all-you-can-move-buffet-edition.685224/)  
Recon Pay (Xtra5) Horse Thieves (v 1) INT  
진짜로 말을 훔쳐탈 수 있게 해주는 건 아니고, 그만큼 정찰 유닛이 말 근처에서 이동력이 늘어납니다.  
...
[Whale Battles (v 1)](https://forums.civfanatics.com/threads/maritime-battles-for-vp.625642/post-16829248)  
Whale Battles (v 1) INT  
Maritime Battles에 추가 예정인 모드로 고래를 사냥하는 포경을 할 수 있게 해주는 모드입니다.  

[World Congress Reformation (v 12)](https://forums.civfanatics.com/threads/world-congress-reformation.557864/)  
World Congress Reformation (v 12) INT  
...
실제 세계의 시작 위치와 사용자 정의 옵션이 포함된 지구 지도 모드입니다.  
※ 모드팩 한정으로 YnAEMP 전용 메뉴를 사용할 수 없어 모드팩에서 제외됩니다.  
...
정부 형태를 고르고 정부가 선호하는 법안과 정부와 관련된 불가사의가 추가되는 모드입니다. [모드 소개와 함께 번역을 해주신 분](https://gall.dcinside.com/mgallery/board/view/?id=civilization&no=275728) 이 계신데, hyuckkim님께서 이 글을 참조했는지는 모릅니다만, 여튼 이 모드를 번역해놓은 모드가 있어 이를 최신 버전에 이식했습니다. 아직 일부 정당명 등이 번역이 덜 되었습니다.  
※ 모드팩에 포함시 건물 건설이 불가한 문제가 있어 모드팩에서 제외됩니다.  

[Really Advanced Setup (v 15)](https://forums.civfanatics.com/threads/really-advanced-setup.486324/) KR  
게임을 시작하기 전의 고급 설정 화면을 훨씬 더 세세하게 바꿔주는 모드입니다.  
※ 모드팩 한정으로 고급 설정 화면이 망가져 모드팩에저 제외됩니다.  

[Recon Pay (Xtra2) Free ZoC (v 3)](https://forums.civfanatics.com/threads/recon-pay-terrain-cost-all-you-can-move-buffet-edition.685224/)  
Recon Pay (Xtra2) Free ZoC (v 3) INT  
이동 중에 통제 구역(ZOC)를 무시합니다.  
...
게임 내에서 상단 패널 UI를 개선해주는 모드입니다. 현재 VP와 호환되지 않아 흔적만 남겨두고 계획은 취소되었습니다.  
