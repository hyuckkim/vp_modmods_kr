...
function AssignStartingPlots:__Init()
	...
	for resource_data in GameInfo.Resources() do
		...
		elseif resourceType == "RESOURCE_OAK" then	-- Padre
...
function AssignStartingPlots:DetermineRegionTypes()
	...
	for this_region, terrainCounts in ipairs(self.regionTerrainCounts) do
		...
		local treesCount = (forestCount*2) + jungleCount --Padre																	
...
function AssignStartingPlots:AddStrategicBalanceResources(region_number)
	...
	local _, horse_amt, _, iron_amt, _, _, _  = self:GetSmallStrategicResourceQuantityValues();
	local shuf_list;
	...
	for _, plotIndex in ipairs(plotList) do
		...
		elseif plotType == PlotTypes.PLOT_HILLS then
			...
			elseif featureType == FeatureTypes.FEATURE_JUNGLE then
				...
				elseif featureType == FeatureTypes.FEATURE_FOREST then
				if plot:IsAdjacentToTerrain(GameInfoTypes.TERRAIN_MOUNTAIN) then 
                        table.insert(tResourceList[PlotListTypes.ADJ_MOUNTAIN_FOREST], plotIndex);
                 end																 
		table.insert(tResourceList[PlotListTypes.MARBLE], plotIndex);
...
function AssignStartingPlots:PlaceStrategicResourceImpact(x, y, radius)
	...
	self:PlaceResourceImpact(x, y, ImpactLayers.LAYER_OAK, radius);		--Padre																								   
end
------------------------------------------------------------------------------
function AssignStartingPlots:CountTotalStrategicResourceImpact(plotIndex)
	return self.impactData[ImpactLayers.LAYER_IRON][plotIndex] +
		...
		self.impactData[ImpactLayers.LAYER_OAK][plotIndex];		--Padre  
end
------------------------------------------------------------------------------
function AssignStartingPlots:IsImpactLayerStrategic(layerIndex)
	return layerIndex == ImpactLayers.LAYER_IRON or
		...
		layerIndex == ImpactLayers.LAYER_URANIUM or
		layerIndex == ImpactLayers.LAYER_OAK;		--Padre
end
------------------------------------------------------------------------------
...
function AssignStartingPlots:PlaceSmallQuantitiesOfStrategics(frequency, plot_list)
	...
	local iNumToPlace = math.ceil(iNumTotalPlots / frequency);
		...
		local uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt, oak_amt = self:GetSmallStrategicResourceQuantityValues();
		...
		if current_index <= iNumTotalPlots then
			for index_to_check = current_index, iNumTotalPlots do
				...
				if self:CountTotalStrategicResourceImpact(plotIndex) == 0 then
					...
					if res_plot:GetResourceType() == -1 then
						...
						if featureType == FeatureTypes.FEATURE_MARSH or featureType == FeatureTypes.FEATURE_JUNGLE then
							...
							if (featureType == FeatureTypes.FEATURE_JUNGLE) and (diceroll < 1) then
										   selected_ID = self.oak_ID;
										   selected_quantity = oak_amt;
							elseif diceroll < 2 or plotType == PlotTypes.PLOT_HILLS then
				...
				elseif featureType == FeatureTypes.FEATURE_FOREST then
							...
							local diceroll = Map.Rand(5, "Resource selection - Place Small Quantities LUA");
							if (diceroll < 2) or res_plot:IsAdjacentToTerrain(GameInfoTypes.TERRAIN_MOUNTAIN) then
							selected_ID = self.oak_ID;
							selected_quantity = oak_amt;
...
function AssignStartingPlots:AddModernMinorStrategicsToCityStates()
	...
	local _, _, oil_amt, _, coal_amt, alum_amt, _ = self:GetSmallStrategicResourceQuantityValues();
	for city_state = 1, self.iNumCityStates do
		-- First check to see if this city state number received a valid start plot.
		if not self.city_state_validity_table[city_state] then
			-- This one did not! It does not exist on the map nor have valid data, so we will ignore it.
...
function AssignStartingPlots:PrintFinalResourceTotalsToLog()
	...
	print(self.oak_ID,  "Timber.: ", self.amounts_of_resources_placed[self.oak_ID + 1])
	print("-");
...
function AssignStartingPlots:GetMajorStrategicResourceQuantityValues()
	...
	local uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt, oak_amt = 2, 3, 5, 3, 4, 7, 3;
	-- Check the strategic deposit size setting.
	if self.resSize == 1 then -- Small
		uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt, oak_amt = 2, 2, 3, 2, 2, 3, 2;
	elseif self.resSize == 3 then -- Large
		uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt, oak_amt = 4, 6, 8, 6, 7, 10, 6;
	end

	if self:IsReducedSupplyActive() then
		print("Reduced Supply Mod is active, halving major strategic supply quantity values")
		uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt, oak_amt = 2, 3, 4, 3, 4, 5, 3;
	end

	return uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt, oak_amt
end
------------------------------------------------------------------------------
function AssignStartingPlots:GetSmallStrategicResourceQuantityValues()
	...
	local uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt, oak_amt = 1, 2, 2, 2, 2, 3, 2;
	-- Check the strategic deposit size setting.
	if self.resSize == 1 then -- Small
		uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt, oak_amt = 1, 1, 1, 1, 1, 2, 1;
	elseif self.resSize == 3 then -- Large
		uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt, oak_amt = 2, 2, 2, 2, 2, 3, 2;
	end

	if self:IsReducedSupplyActive() then
		print("Reduced Supply Mod is active, halving small strategic supply quantity values")
		uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt, oak_amt = 1, 2, 2, 2, 2, 3, 2;
	end
...
function AssignStartingPlots:PlaceStrategicAndBonusResources()
	...
	local uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt, oak_amt = self:GetMajorStrategicResourceQuantityValues();
	local resources_to_place = {};
	...
	if self:IsReducedSupplyActive() then
    resMultiplier = resMultiplier*1.2
	end
    ...
    local iW, iH = Map.GetGridSize();
    local sizeadjust = rounddecimal(10/(iW*iH), 4)
	local resMultiplierOak = rounddecimal (math.min(1.5, math.max ((resMultiplier * NUM_IMPACT_TREES * sizeadjust), 0.66666667)), 8)
	
print("Map Generation - Timber ResMultiplier = ", resMultiplierOak);
	


		resources_to_place = {
		{self.oak_ID, oak_amt, 100, 1, 2}
	};
	self:ProcessResourceList(10 * resMultiplierOak, ImpactLayers.LAYER_OAK, tPlotList[PlotListTypes.TUNDRA_FOREST], resources_to_place);
	self:ProcessResourceList(15 * resMultiplierOak, ImpactLayers.LAYER_OAK, tPlotList[PlotListTypes.ADJ_MOUNTAIN_FOREST], resources_to_place);
	self:ProcessResourceList(20 * resMultiplierOak, ImpactLayers.LAYER_OAK, tPlotList[PlotListTypes.HILLS_FOREST], resources_to_place);
	self:ProcessResourceList(25 * resMultiplierOak, ImpactLayers.LAYER_OAK, tPlotList[PlotListTypes.FLAT_FOREST], resources_to_place);
	

    resources_to_place = {
		{self.oak_ID, oak_amt * 0.7, 100, 2, 3}
	};
	self:ProcessResourceList(30 * resMultiplierOak, ImpactLayers.LAYER_OAK, tPlotList[PlotListTypes.HILLS_JUNGLE], resources_to_place);
	self:ProcessResourceList(38 * resMultiplierOak, ImpactLayers.LAYER_OAK, tPlotList[PlotListTypes.FLAT_JUNGLE], resources_to_place);
	...
	uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt, oak_amt = self:GetSmallStrategicResourceQuantityValues();
local iLoop = 0;
	while self.amounts_of_resources_placed[self.oak_ID + 1] < 5 * self.iNumCivs and iLoop < 999 do
		print("Map has very low Timber, adding another.");
		local resources_to_place = { {self.oak_ID, oak_amt, 25, 1, 2} };
		self:ProcessResourceList(99999, ImpactLayers.LAYER_OAK, tPlotList[PlotListTypes.ADJ_MOUNTAIN_FOREST], resources_to_place);
		self:ProcessResourceList(99999, ImpactLayers.LAYER_OAK, tPlotList[PlotListTypes.TUNDRA_FOREST], resources_to_place);
		self:ProcessResourceList(99999, ImpactLayers.LAYER_OAK, tPlotList[PlotListTypes.FLAT_FOREST], resources_to_place);
		self:ProcessResourceList(99999, ImpactLayers.LAYER_OAK, tPlotList[PlotListTypes.HILLS_FOREST], resources_to_place);
		self:ProcessResourceList(99999, ImpactLayers.LAYER_OAK, tPlotList[PlotListTypes.FLAT_JUNGLE], resources_to_place);
		self:ProcessResourceList(99999, ImpactLayers.LAYER_OAK, tPlotList[PlotListTypes.HILLS_JUNGLE], resources_to_place);
	iLoop = iLoop + 1;
	end
	iLoop = 0;
