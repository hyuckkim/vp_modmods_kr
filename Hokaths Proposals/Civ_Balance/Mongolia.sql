...
WHERE bc.Type IN ('BUILDINGCLASS_STABLE', 'BUILDINGCLASS_AGRIBUSINESS') AND
...
DELETE FROM Civilization_BuildingClassOverrides WHERE BuildingClassType = 'BUILDINGCLASS_SMOKEHOUSE' AND CivilizationType = 'CIVILIZATION_MONGOL';
...
-- +2 [ICON_FOOD] Food, [ICON_CULTURE] Culture, and [ICON_CULTURE_LOCAL] Border Growth Points, and [ICON_GREAT_GENERAL] Great General Points from [ICON_MUSICIAN] Musicians and Pastures worked by the City. +3 [ICON_CULTURE] Culture, [ICON_PEACE] Faith, and [ICON_TOURISM] Tourism for every Mountain within the workable tiles.[NEWLINE][NEWLINE]Nearby Grassland: +1 [ICON_FOOD] Food and [ICON_TOURISM] Tourism.[NEWLINE]Nearby Plains: +1 [ICON_PRODUCTION] Production and [ICON_PEACE] Faith.[NEWLINE][NEWLINE]1 Specialist in this City no longer produces [ICON_HAPPINESS_3] Unhappiness from [ICON_URBANIZATION] Urbanization.[NEWLINE][NEWLINE]Maximum of 3 of these Buildings in your Empire.

------------------------------------------------
--UPDATE Building_YieldChanges SET
--YieldType = 'YIELD_GOLD'
--WHERE BuildingType = 'BUILDING_GER' AND Yield = 1;

--UPDATE Language_en_US
--SET Text = 'Unique MONGOLIA replacement for the {TXT_KEY_BUILDING_SMOKEHOUSE}. In addition to the {TXT_KEY_BUILDING_SMOKEHOUSE}''s bonuses, {TXT_KEY_BUILDING_GER} generates [ICON_GOLD] Gold; boosts pastures; provides [ICON_PRODUCTION] Production when borders expand; and makes borders expand faster.'
