...
BEGIN
	INSERT INTO Building_BuildingClassLocalYieldChanges
		(BuildingType, BuildingClassType, YieldType, YieldChange)
	SELECT
		NEW.Type, 'BUILDINGCLASS_ORPHANAGE', 'YIELD_GOLD', 3;
END;
...
WHERE a.Type IN ('BUILDING_FACTORY', 'BUILDING_AGRIBUSINESS', 'BUILDING_SEAPORT')
...
VALUES
	('BUILDING_ORPHANAGE', 'IMPROVEMENT_ACADEMY', 	            'YIELD_FAITH', 3),
	('BUILDING_ORPHANAGE', 'IMPROVEMENT_CUSTOMS_HOUSE',  	    'YIELD_FOOD', 4),
	('BUILDING_ORPHANAGE', 'IMPROVEMENT_HOLY_SITE',  	    'YIELD_GOLD', 3),
...
VALUES
	('BUILDING_ORPHANAGE', 'BUILDINGCLASS_FACTORY', 1);
...
VALUES	
	...
	('TXT_KEY_BUILDING_ORPHANAGE_HELP', '');

-- 25% of the [ICON_PEACE] Faith output of the City is added as an instant boost whenever a [ICON_CITIZEN] Citizen is born in this City. +4 [ICON_FOOD] Food from Towns and Holy Sites. +1 [ICON_HAPPINESS_1] Happiness from Factories in the City. Factories, Agribusinesses, Seaports, and Manufactories in the City gain +3 [ICON_PRODUCTION] Production. +3 [ICON_GOLD] Gold from Academies and every Building in the City that must be purchased with [ICON_PEACE] Faith.
