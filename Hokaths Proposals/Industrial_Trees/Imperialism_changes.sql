...
VALUES
	('POLICY_MARITIME_INFRASTRUCTURE', 'POLICY_NAVIGATION_SCHOOL'),	('POLICY_MARITIME_INFRASTRUCTURE', 'POLICY_TREASURE_FLEETS'),
	('POLICY_MERCHANT_NAVY', 'POLICY_NAVAL_TRADITION');

-- Naval Tradition (now Colonialism)
DELETE FROM Policy_ImprovementYieldChanges WHERE PolicyType = 'POLICY_NAVAL_TRADITION';

-- note: are there any other uniques we need here?
INSERT INTO Policy_ImprovementYieldChanges
	(PolicyType, ImprovementType, YieldType, Yield)
VALUES	
	('POLICY_NAVAL_TRADITION', 'IMPROVEMENT_MANUFACTORY', 'YIELD_SCIENCE', 2),
	('POLICY_NAVAL_TRADITION', 'IMPROVEMENT_MANUFACTORY', 'YIELD_CULTURE', 2),
	('POLICY_NAVAL_TRADITION', 'IMPROVEMENT_MANUFACTORY', 'YIELD_GREAT_ADMIRAL_POINTS', 2),
	('POLICY_NAVAL_TRADITION', 'IMPROVEMENT_CITADEL', 'YIELD_SCIENCE', 2),
	('POLICY_NAVAL_TRADITION', 'IMPROVEMENT_CITADEL', 'YIELD_CULTURE', 2),
	('POLICY_NAVAL_TRADITION', 'IMPROVEMENT_CITADEL', 'YIELD_GREAT_ADMIRAL_POINTS', 2),
	('POLICY_NAVAL_TRADITION', 'IMPROVEMENT_ORDO', 'YIELD_SCIENCE', 2),
	('POLICY_NAVAL_TRADITION', 'IMPROVEMENT_ORDO', 'YIELD_CULTURE', 2),
	('POLICY_NAVAL_TRADITION', 'IMPROVEMENT_ORDO', 'YIELD_GREAT_ADMIRAL_POINTS', 2);

INSERT INTO Policy_ImprovementYieldChanges
	(PolicyType, ImprovementType, YieldType, Yield)
SELECT
	'POLICY_NAVAL_TRADITION', 'IMPROVEMENT_ZULU_KRAAL', Type, 2
FROM Yields WHERE Type IN ('YIELD_SCIENCE', 'YIELD_CULTURE', 'YIELD_GREAT_ADMIRAL_POINTS')
AND EXISTS (SELECT * FROM Improvements WHERE Type = 'IMPROVEMENT_ZULU_KRAAL');

-- Merchant Navy (now Exploitation)
DELETE FROM Policy_ImprovementYieldChanges WHERE PolicyType = 'POLICY_MERCHANT_NAVY';

INSERT INTO Policy_ResourceYieldChanges
	(PolicyType, ResourceType, YieldType, Yield)
SELECT
	'POLICY_MERCHANT_NAVY', Type, 'YIELD_FOOD', 3
FROM Resources
WHERE Happiness > 0;
...
INSERT INTO Policy_ResourceYieldChanges
	(PolicyType, ResourceType, YieldType, Yield)
SELECT
	'POLICY_MERCHANT_NAVY', Type, 'YIELD_CULTURE_LOCAL', 3
FROM Resources
WHERE Happiness > 0;
...
INSERT INTO Policy_ResourceYieldChanges
	(PolicyType, ResourceType, YieldType, Yield)
SELECT
	'POLICY_MERCHANT_NAVY', Type, 'YIELD_PRODUCTION', 3
FROM Resources
WHERE Happiness > 0;
...
INSERT INTO Policy_ImprovementYieldChanges
	(PolicyType, ImprovementType, YieldType, Yield)
VALUES	
	('POLICY_MARITIME_INFRASTRUCTURE', 'IMPROVEMENT_FORT', 'YIELD_CULTURE_LOCAL', 2),
	('POLICY_MARITIME_INFRASTRUCTURE', 'IMPROVEMENT_FORT', 'YIELD_CULTURE', 2);

UPDATE Policies
SET
PuppetYieldPenaltyMod = 30
WHERE Type = 'POLICY_MARITIME_INFRASTRUCTURE';

-- Finisher
UPDATE Resources
SET PolicyReveal = 'POLICY_EXPLORATION_FINISHER'
...
UPDATE Language_en_US
SET Text = '[COLOR_POSITIVE_TEXT]Colonialism[ENDCOLOR][NEWLINE][ICON_BULLET]+2 [ICON_RESEARCH] Science and +1 [ICON_CULTURE] Culture from Barracks, Armories, Military Academies.[NEWLINE][ICON_BULLET]+2 [ICON_RESEARCH] Science, [ICON_CULTURE] Culture, and [ICON_GREAT_ADMIRAL] Great Admiral Points from Citadels and Manufactories.[NEWLINE][ICON_BULLET]Each unique [ICON_MONOPOLY] Global Monopoly modifier is increased by an additional 10% if it''s percentage-based, or +3 otherwise.'
