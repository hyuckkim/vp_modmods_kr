...
function OnViewGovernmentsButton()
	LuaEvents.UI_ShowChooseGovernmentPopup(true)
end-------------------------------------------------
-------------------------------------------------
function OnChangeGovernmentsButton()
...
function RefreshYourGovernment()
	...
	if (not player:IsHasGovernment()) then
		...
		Controls.SovereigntyInfoStack1:SetHide(true)
		Controls.SovereigntyInfoStack2:SetHide(true)
	...
	elseif player:IsAnarchy() then
		...
		Controls.SovereigntyInfoStack1:SetHide(false)
		Controls.SovereigntyInfoStack2:SetHide(false)
		...
		--VIEW GOVERNMENTS
		local canChangeGovernment, strCanChangeGovernment = player:CanChangeGovernment()
		if canChangeGovernment then
			Controls.ViewGovernmentsButton:SetHide(true)
			Controls.ViewGovernmentsAnim:SetHide(false)
			Controls.ViewGovernmentsFlashing:LocalizeAndSetText("TXT_KEY_JFD_GOVERNMENT_OVERVIEW_CHANGE_GOVERNMENTS")
			Controls.ViewGovernmentsFlashing:RegisterCallback(Mouse.eLClick, OnChangeGovernmentsButton)
		else
			Controls.ViewGovernmentsButton:SetHide(false)
			Controls.ViewGovernmentsAnim:SetHide(true)
			Controls.ViewGovernmentsButton:SetText(Locale.ConvertTextKey("TXT_KEY_JFD_GOVERNMENT_OVERVIEW_VIEW_GOVERNMENTS"))
			Controls.ViewGovernmentsButton:RegisterCallback(Mouse.eLClick, OnViewGovernmentsButton)
		end
		Controls.ViewGovernmentsButton:SetToolTipString(strCanChangeGovernment)
		Controls.ViewGovernmentsAnim:SetToolTipString(strCanChangeGovernment)
		...
		--RELIGION
		local religionID = player:GetMainReligion(true)
		...
		Controls.HeadOfGovernment:LocalizeAndSetText(dominantFaction.Adjective)
		...
		if numFreeReforms > 0 then
			Controls.FreeReformsLabel:LocalizeAndSetText("TXT_KEY_JFD_GOVERNMENT_OVERVIEW_FREE_REFORMS_LABEL", numFreeReforms)
			Controls.FreeReformsLabel:LocalizeAndSetToolTip("TXT_KEY_JFD_GOVERNMENT_OVERVIEW_FREE_REFORMS_LABEL_TT", numFreeReforms)
			Controls.FreeReformsLabelAni:LocalizeAndSetToolTip("TXT_KEY_JFD_GOVERNMENT_OVERVIEW_FREE_REFORMS_LABEL_TT", numFreeReforms)
		...
		--REFORM COOLDOWN
		local numReformCooldown, numCurrentReformCooldown, strReformCooldownTT = player:CalculateReformCooldown(true)
		Controls.ReformCooldownTurnLabel:LocalizeAndSetText("TXT_KEY_JFD_GOVERNMENT_OVERVIEW_NEXT_REFORM_TURN_LABEL", numCurrentReformCooldown)
		Controls.ReformCooldownTurnLabel:LocalizeAndSetToolTip(strReformCooldownTT)
		...
		Controls.RevolutionarySentiment:LocalizeAndSetText("TXT_KEY_JFD_GOVERNMENT_OVERVIEW_REVOLUTIONARY_SENTIMENT", numRevolutionarySentiment) 
		Controls.RevolutionarySentiment:LocalizeAndSetToolTip("TXT_KEY_JFD_GOVERNMENT_OVERVIEW_REVOLUTIONARY_SENTIMENT_TT", strRevolutionarySentimentModTT) 
		
		--SOVEREIGNTY
		local numCurrentSov = player:GetCurrentSovereignty()
		local strSovTT = Player_GetSovereigntyTopPanelInfoTT(player)
		Controls.CurrentSovereigntyLabel:LocalizeAndSetText("TXT_KEY_JFD_GOVERNMENT_OVERVIEW_CURRENT_SOVEREIGNTY_LABEL", numCurrentSov)
		Controls.CurrentSovereigntyLabel:LocalizeAndSetToolTip(strSovTT) 
	
		Controls.SovereigntyInfoStack1:ReprocessAnchoring()
...
function RefreshCouncillors()
	...
	for index = 1, #councillorsTable do
		...
		if player:IsHasCouncillor(councillorID) then
			local councillorName = player:GetCouncillorName(councillorID)
    		...
    		strCouncillorTT = strCouncillorTT .. Locale.ConvertTextKey(row.Help, "[COLOR_POSITIVE_TEXT]", "")
            strCouncillorTT = strCouncillorTT .. Locale.ConvertTextKey("TXT_KEY_COUNCILLOR_JFD_HELP_FILLED_NOTE")

        	local councillorUnitClass = GameInfo.JFD_Councillors[councillorID].UnitClassType
            ...
            thisButton:ClearCallback( Mouse.eLClick )
		else	
            ...
            strCouncillorTT = strCouncillorTT .. Locale.ConvertTextKey("TXT_KEY_COUNCILLOR_JFD_HELP_VACANT")
   			strCouncillorTT = strCouncillorTT .. Locale.ConvertTextKey(row.Help, "[COLOR_FADING_POSITIVE_TEXT]", "[COLOR_JFD_ALPHA]")
            ...
            thisButton:RegisterCallback(Mouse.eLClick, function() 
                LuaEvents.ChooseCouncillor(row)
                OnClose()
            end)
		end		
...
local g_ShowAbsentFactions = true
...
function RefreshLegislature()
	...
	local player = Players[Game.GetActivePlayer()]
	local dominantFactionID, dominantFactionPower = player:GetDominantFaction()
	local factionInfo = GameInfo.JFD_Factions[dominantFactionID]
	local mainReligionID = player:GetMainReligion()
	local religion = GameInfo.Religions[mainReligionID]
	if (dominantFactionPower >= 100 and (not g_ShowAbsentFactions)) then
		local factionInfo = GameInfo.JFD_Factions[dominantFactionID]
		local factionName = player:GetFactionName(dominantFactionID)
		local factionHelp = player:GetFactionHelp(dominantFactionID, factionName, dominantFactionPower)
			
		Controls.DictatorshipLabel:SetHide(false)
		Controls.DictatorshipLabelPercent:SetHide(false)
		Controls.FactionScrollPanel:SetHide(true)
		Controls.DictatorshipLabel:LocalizeAndSetToolTip(factionHelp)
		Controls.DictatorshipLabel:SetText(factionInfo.IconString .. " The " .. factionName)
		Controls.DictatorshipLabelPercent:SetText("[COLOR_JFD_SOVEREIGNTY]" .. dominantFactionPower .. "%[ICON_JFD_SOVEREIGNTY][ENDCOLOR]")
		Controls.DictatorshipStack:ReprocessAnchoring()

		if factionInfo.IsMandateOfHeavenLoss then
			Controls.DictatorshipLabelPercent:SetHide(true)
		end
	else
		Controls.FactionScrollPanel:SetHide(false)
		Controls.DictatorshipLabel:SetHide(true)
		Controls.DictatorshipLabelPercent:SetHide(true)
		...
		for _, faction in ipairs(theseFactionsTable) do
			...
			local factionHL = "[COLOR_JFD_SOVEREIGNTY_FADING]"
			if dominantFactionID == factionID then
				factionHL = "[COLOR_JFD_SOVEREIGNTY]" 
			end
			instance.FactionName:LocalizeAndSetToolTip(factionHelp)
			instance.Sovereignty:LocalizeAndSetText("{1_HL}{2_num}%[ICON_JFD_SOVEREIGNTY][ENDCOLOR]", factionHL, factionPower)
...
function GetReformHelpToolTip(control)
	...
	local strReformHelp = Player_GetReformInfoTT(player, reformID, reformCost)
	...
	if player:IsHasGovernment() then
		...
		for index = 1, numReforms do
			...
			if row.ReformBranchType == currentReformBranchType then
				...
				local reformHelp = Player_GetReformInfoTT(player, reformID, reformCost)
...
function OnReformClicked(reformID)
	...
	Controls.ReformConfirmLabel:LocalizeAndSetText("TXT_KEY_JFD_GOVERNMENT_OVERVIEW_CONFIRM_REFORM", reform.ShortDescription, reform.ReformSubBranchDescription)
	Controls.ConfirmGrid:SetSizeVal(500,310)
	...
	for playerID = 0, defineMaxMajorCivs - 1 do	
		...
		if player:IsAlive() and ((playerTeam:IsHasMet(teamID) and (not g_IsCBRTestActive)) or g_IsCBRTestActive) and (not player:IsBarbarian()) then
			...
			if governmentID ~= -1 then
				...
				local factionDesc = "TXT_KEY_JFD_GOVERNMENT_OVERVIEW_FACTION_PANEL_NONE"
				local factionTT = "TXT_KEY_JFD_GOVERNMENT_OVERVIEW_FACTION_PANEL_NONE"
				...
				if factionDominantID > -1 then	
					...
					factionDesc = faction.IconString .. " " .. Locale.ConvertTextKey(factionName)
				...
				local cyclePowerDesc = "TXT_KEY_JFD_GOVERNMENT_OVERVIEW_CYCLE_POWER_PANEL_NONE"
				...
				FactionType = factionDesc,
				FactionTT = factionTT,
				CyclePowerType = cyclePowerDesc,}
	...
	if(#worldGovernmentsTable > 0) then
		...
		for i,v in ipairs(worldGovernmentsTable) do
			...
			if player:IsAnarchy() then
				IconHookup(9, 48, government.AlphaAtlas, instance.GovernmentTypeIcon)
			else
...
function ShowNewLegislaturePopup( playerID, governmentID, strLegislature, strNewFactionCityResult, isShowLegislatureComposition )
	...
	for _, faction in ipairs(theseFactionsTable) do
		...
		instance.FactionName:LocalizeAndSetToolTip(factionHelp)
		instance.Sovereignty:LocalizeAndSetText("{1_HL}{2_num}%[ICON_JFD_SOVEREIGNTY][ENDCOLOR]", factionHL, factionPower)
		local strResultsTT = strNewFactionCityResult[factionID]
		if strResultsTT then
			instance.Sovereignty:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_JFD_GOVERNMENT_OVERVIEW_FACTION_POWER_TT_RESULT") .. Locale.ConvertTextKey(strResultsTT))
		else
