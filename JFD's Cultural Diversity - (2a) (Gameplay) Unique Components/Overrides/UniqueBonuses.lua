...
function PopulateUniqueBonuses( controlTable, civ, _, extendedTooltip, noTooltip)
	...
	print("we have called the VP Override Populate");
	...
	function PopulateButton(button, buttonFrame)
		...
		for row in DB.Query([[SELECT DISTINCT i.ID, i.Description, i.PortraitIndex, i.IconAtlas from Improvements i, Civilization_Leaders cl, Leader_Traits lt, Trait_BuildsUnitClasses tbuc, Builds b where cl.CivilizationType = ? and lt.LeaderType = cl.LeaderheadType and lt.TraitType = tbuc.TraitType and tbuc.BuildType = b.Type and i.Type = b.ImprovementType and b.OrderPriority=90]], civ.Type) do
			AdjustArtOnUniqueImprovementButton( button, buttonFrame, row, textureSize, extendedTooltip, noTooltip);
 			button, buttonFrame = coroutine.yield(row.Description);
		end
...
function PopulateUniqueBonuses_CreateCached()
	...
	local culturalImprovementsQuery = DB.CreateQuery([[SELECT DISTINCT i.ID, i.Description, i.PortraitIndex, i.IconAtlas from Improvements i, Civilization_Leaders cl, Leader_Traits lt, Trait_BuildsUnitClasses tbuc, Builds b where cl.CivilizationType = ? and lt.LeaderType = cl.LeaderheadType and lt.TraitType = tbuc.TraitType and tbuc.BuildType = b.Type and i.Type = b.ImprovementType and b.OrderPriority=90]]);
