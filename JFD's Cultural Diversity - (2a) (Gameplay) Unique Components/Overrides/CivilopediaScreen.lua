...
-- Vox Populi end
		...
		local replacesBuildingClass = {};
		local specificCivs = {};

		local classOverrideCondition = string.format("BuildingType='%s' and CivilizationType <> 'CIVILIZATION_BARBARIAN' and CivilizationType <> 'CIVILIZATION_MINOR'", thisBuilding.Type);
		for row in GameInfo.Civilization_BuildingClassOverrides(classOverrideCondition) do
			specificCivs[row.CivilizationType] = 1;
			replacesBuildingClass[row.BuildingClassType] = 1;
		end

		g_ReplacesManager:ResetInstances();
		buttonAdded = 0;

		for buildingClassType, _ in pairs(replacesBuildingClass) do
			for replacedBuilding in DB.Query("SELECT u.ID, u.Description, u.PortraitIndex, u.IconAtlas from Buildings as u inner join BuildingClasses as uc on u.Type = uc.DefaultBuilding where uc.Type = ?", buildingClassType) do
				local thisBuildingInstance = g_ReplacesManager:GetInstance();
				if thisBuildingInstance then
					local textureOffset, textureSheet = IconLookup( replacedBuilding.PortraitIndex, buttonSize, replacedBuilding.IconAtlas );
					if textureOffset == nil then
						textureSheet = defaultErrorTextureSheet;
						textureOffset = nullOffset;
					end
					UpdateSmallButton( buttonAdded, thisBuildingInstance.ReplaceImage, thisBuildingInstance.ReplaceButton, textureSheet, textureOffset, CategoryBuildings, Locale.ConvertTextKey( replacedBuilding.Description ), replacedBuilding.ID );
					buttonAdded = buttonAdded + 1;
				end
			end
